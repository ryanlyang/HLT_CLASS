"""Versioned artifacts for the full-data TRI100 four-spine study."""

from __future__ import annotations

from typing import Any, Final, Mapping

from hlt_classification.data.cache_contracts import (
    require_sha256, validate_content_hash, with_content_hash,
)


PREFIX: Final = "HCWDL_TRI100_FOUR_SPINE_LOGIT"
# Version 2 is the immutable single-GPU successor to the cancelled v1
# four-node DDP execution.  The scientific ladder is unchanged, but execution
# identity is intentionally not interchangeable with v1.
GRAPH_CONTRACT: Final = f"{PREFIX}_GRAPH/v2"
NODE_CONTRACT: Final = f"{PREFIX}_NODE_SPEC/v2"
RECIPE_CONTRACT: Final = f"{PREFIX}_RECIPE/v2"
SOURCE_LOCK_CONTRACT: Final = f"{PREFIX}_SOURCE_LOCK/v2"
EXECUTION_ACCEPTANCE_CONTRACT: Final = f"{PREFIX}_EXECUTION_ACCEPTANCE/v2"
SPEC_CONTRACT: Final = f"{PREFIX}_CAMPAIGN_SPEC/v2"
PLAN_CONTRACT: Final = f"{PREFIX}_COMMAND_PLAN/v2"
TRAINING_REPORT_CONTRACT: Final = f"{PREFIX}_TRAINING_REPORT/v2"
SELECTED_CHECKPOINT_CONTRACT: Final = f"{PREFIX}_SELECTED_CHECKPOINT/v2"
FINAL_CHECKPOINT_CONTRACT: Final = f"{PREFIX}_FINAL_CHECKPOINT/v2"
PROBABILITY_SHARD_CONTRACT: Final = f"{PREFIX}_PROBABILITY_SHARD/v2"
PROBABILITY_MANIFEST_CONTRACT: Final = f"{PREFIX}_PROBABILITY_MANIFEST/v2"
PROBABILITY_LOCK_CONTRACT: Final = f"{PREFIX}_PROBABILITY_LOCK/v2"
STAGE_REPORT_CONTRACT: Final = f"{PREFIX}_STAGE_REPORT/v2"
AGGREGATE_CONTRACT: Final = f"{PREFIX}_AGGREGATE/v2"
COMPLETE_CONTRACT: Final = f"{PREFIX}_CAMPAIGN_COMPLETE/v2"
RECOVERY_SPEC_CONTRACT: Final = f"{PREFIX}_RECOVERY_SPEC/v2"
MONITOR_CONTRACT: Final = f"{PREFIX}_MONITOR/v2"
SCHEMA_VERSION: Final = 1

CONTRACTS: Final = (
    GRAPH_CONTRACT, NODE_CONTRACT, RECIPE_CONTRACT, SOURCE_LOCK_CONTRACT,
    EXECUTION_ACCEPTANCE_CONTRACT,
    SPEC_CONTRACT, PLAN_CONTRACT, TRAINING_REPORT_CONTRACT,
    SELECTED_CHECKPOINT_CONTRACT, FINAL_CHECKPOINT_CONTRACT,
    PROBABILITY_SHARD_CONTRACT, PROBABILITY_MANIFEST_CONTRACT,
    PROBABILITY_LOCK_CONTRACT, STAGE_REPORT_CONTRACT, AGGREGATE_CONTRACT,
    COMPLETE_CONTRACT, RECOVERY_SPEC_CONTRACT, MONITOR_CONTRACT,
)


def artifact(payload: Mapping[str, Any], *, contract: str) -> dict[str, Any]:
    if contract not in CONTRACTS:
        raise ValueError("unknown TRI100 four-spine contract")
    return with_content_hash({
        **dict(payload), "contract": contract, "schema_version": SCHEMA_VERSION,
    })


def validate_artifact(value: Mapping[str, Any], *, contract: str) -> str:
    if contract not in CONTRACTS:
        raise ValueError("unknown TRI100 four-spine contract")
    return validate_content_hash(
        value, expected_contract=contract,
        expected_schema_version=SCHEMA_VERSION,
    )


def hashes(values: Mapping[str, str]) -> dict[str, str]:
    if not values:
        raise ValueError("TRI100 four-spine parent registry is empty")
    return {
        str(name): require_sha256(value, name=str(name))
        for name, value in sorted(values.items())
    }


__all__ = [name for name in globals() if name.endswith("_CONTRACT")] + [
    "CONTRACTS", "SCHEMA_VERSION", "artifact", "hashes", "validate_artifact",
]
