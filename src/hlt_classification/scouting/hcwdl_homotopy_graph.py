"""Exact 45-fit v2 graph and recipe overlay for the HCWDL-UJ study."""
...
def _build_domains() -> dict[str, dict[str, object]]:
    ...
    for index in range(1, 6):
        result[f"u{index * 20:03d}"] = _domain("privileged", index / 5, 0.0)
        result[f"d{100-index * 20}f"] = _domain("privileged", 1.0, index / 5,
                                                 deployable=index == 5)
    for index in range(1, 11):
        result[f"j{index * 10:03d}"] = _domain(
            "privileged" if index < 10 else "hlt", index / 10, index / 10,
            deployable=index == 10,
        )
    return result
...
def _build_registry() -> dict[str, HomotopyNodeSpec]:
    ...
    add("U020P0KD", track="control", stage="adapter", domain="u020",
        teacher=("P0KD", "p0"), transition=1, seed_alias="transition_01")
    add("D100direct", track="control", stage="direct", domain="d100",
        teacher=("TOFF", "toff"), transition=5, seed_alias="transition_05")
    ...
    add("M0self", track="control", stage="born_again", domain="hlt",
        teacher=("M0paired", "hlt"), transition=11, seed_alias="transition_11",
        temperature=1.0)

    previous, previous_domain = "TOFF", "toff"
    for index in range(1, 6):
        node_id, domain = f"U{index * 20:03d}", f"u{index * 20:03d}"
    ...
    for index in range(1, 6):
        ...
        add(node_id, track="factorized", stage="down", domain=domain,
            teacher=(previous, previous_domain), transition=5 + index,
            seed_alias=f"transition_{5 + index:02d}")
        previous, previous_domain = node_id, domain
    add("M1F", track="factorized", stage="born_again", domain="hlt",
        teacher=("D0F", "hlt"), transition=11, seed_alias="transition_11",
        temperature=1.0)

    previous, previous_domain = "TOFF", "toff"
    for index in range(1, 11):
        node_id, domain = f"J{index * 10:03d}", f"j{index * 10:03d}"
        add(node_id, track="joint", stage="joint", domain=domain,
            teacher=(previous, previous_domain), transition=index,
            seed_alias=f"transition_{index:02d}")
        previous, previous_domain = node_id, domain
    add("M1J", track="joint", stage="born_again", domain="hlt",
        teacher=("J100", "hlt"), transition=11, seed_alias="transition_11",
        temperature=1.0)
    ...
    for index in range(1, 12):
        ...
        add(node_id, track="stationary_hlt", stage="stationary", domain="hlt",
            teacher=(previous, previous_domain), transition=index,
            seed_alias=f"transition_{index:02d}",
            temperature=1.0 if index == 11 else 2.0)
...
def validate_graph(
    ...
    if len(registry) != FIT_COUNT or len(set(registry)) != FIT_COUNT:
        raise ValueError("HCWDL-UJ v2 graph must contain exactly 45 new fits")
    expected_tracks = {
        "control": 7, "factorized": 11, "joint": 11,
        "stationary_d100": 5, "stationary_hlt": 11,
    }
    ...
    if NODE_REGISTRY["U020"].teachers[0] != TeacherSpec("TOFF", "toff", "sole"):
        raise ValueError("U020 must receive native TOFF targets")
    if NODE_REGISTRY["J010"].teachers[0] != TeacherSpec("TOFF", "toff", "sole"):
        raise ValueError("J010 must receive native TOFF targets")
    if NODE_REGISTRY["D80F"].teachers[0].node_id != "U100":
    ...
    if any(node.temperature != (1.0 if node.node_id in {"M1F", "M1J", "M0self", "S0_11"} else 2.0)
...
def resolved_loss(node_id: str) -> LossConfiguration:
    ...
    # S0_02..S0_10 are deliberately T2 despite having HLT-domain teachers.
...
__all__ = [
    "DOMAINS", "FIT_COUNT", "GRAPH_LABEL", "GRAPH_SHA256", "HOMOTOPY_TRANSITIONS", "HomotopyNodeSpec",
