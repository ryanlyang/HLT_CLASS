...
def build_aggregate(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for ensemble_id in ENSEMBLE_COMPONENTS:
        ...
        rows.append({
            ...
            "components": list(ENSEMBLE_COMPONENTS[ensemble_id]),
        })
    ...
    d000 = [by_id[name]["metrics"]["macro_ovr_auc"] for name in ENSEMBLE_COMPONENTS["D000E"]]
    comparisons = {
        "D000E_minus_best_D000_auc": by_id["D000E"]["metrics"]["macro_ovr_auc"] - max(d000),
        "D000E_minus_local_D000_auc": by_id["D000E"]["metrics"]["macro_ovr_auc"] - by_id["D000_from_D033E"]["metrics"]["macro_ovr_auc"],
        "D000E_minus_M0paired_auc": by_id["D000E"]["metrics"]["macro_ovr_auc"] - m0["macro_ovr_auc"],
        "M1_minus_D000E_auc": by_id["M1"]["metrics"]["macro_ovr_auc"] - by_id["D000E"]["metrics"]["macro_ovr_auc"],
        "M1_minus_best_D000_auc": by_id["M1"]["metrics"]["macro_ovr_auc"] - max(d000),
...
class MhpeWorkflow:
    ...
    def run(self, task_id: str, *, device: str = "cuda") -> dict[str, Any]:
        ...
        if task["kind"] == "finalist_lock":
            ...
            validate_content_hash(aggregate, expected_contract=AGGREGATE_CONTRACT, expected_schema_version=1)
            by_id = {row["node_id"]: row for row in aggregate["rows"]}
            entries = []
            for node_id in FINALISTS:
                row = by_id[node_id]
                entries.append({
                    "node_id": node_id,
            ...
            output = finalist_lock_payload(aggregate_sha256=aggregate["content_hash"], entries=entries)
