"""Task dispatch and durable outputs for the TRI60 D000 budget screen."""
...
from .hcwdl_tri60_d000_budget_screen_reporting import (
    build_aggregate, build_campaign_complete, training_report,
)
from .hcwdl_tri60_d000_budget_screen_runner import run_fit
...
def _task(task_id: str) -> Mapping[str, Any]:
    try:
        ...
        raise KeyError("unknown TRI60 D000 budget-screen task") from error
...
class D000BudgetScreenWorkflow:
    ...
    def run(self, task_id: str, *, device: str = "cuda") -> dict[str, Any]:
        ...
        if task["kind"] == "authenticate":
            ...
            if validate_source_lock(source) != self.spec["parents"]["source_lock"]:
                raise ValueError("TRI60 D000 budget-screen source changed")
            return source
        if task["kind"] == "preflight":
            ...
            return endpoint
        if task["kind"] == "train":
            return run_fit(spec=self.spec, node_id=task["node_id"], device=device)
        if task["kind"] == "aggregate":
        ...
        if task["kind"] == "campaign_complete":
            ...
            return value
        raise KeyError("unknown TRI60 D000 budget-screen task kind")


__all__ = ["D000BudgetScreenWorkflow", "task_outputs"]
