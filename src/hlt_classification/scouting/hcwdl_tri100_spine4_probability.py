"""Single-model probability banks for the TRI100 four-spine study."""
...
def _identities(value: np.ndarray) -> np.ndarray:
    ...
    if result.dtype != np.uint8 or result.ndim != 2 or result.shape[1] != 32:
        raise ValueError("TRI100 four-spine identities must be uint8 [rows,32]")
    if len({bytes(row) for row in result}) != len(result):
        raise ValueError("TRI100 four-spine identities repeat")
    return result
...
def expected_temperature(role: str) -> float:
    ...
    if role == "validation":
        return 1.0
    raise PermissionError("TRI100 four-spine probability role differs")
...
def publish_probability_role(
    ...
    if re.fullmatch(r"[0-9a-f]{40}", producer_commit) is None:
        raise ValueError("TRI100 four-spine producer commit differs")
    components = PROBABILITY_COMPONENTS.get(distribution_id)
    if components is None or tuple(component_logits) != components:
        raise ValueError("TRI100 four-spine probability component differs")
    ...
    if probabilities.shape != (len(identities), 15):
        raise ValueError("TRI100 four-spine probability rows differ")
    lineage = {}
    for component in components:
        ...
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ValueError("TRI100 four-spine component lineage differs")
        lineage[component] = hashes(row)
    root = Path(output)
...
def load_probability_role(
    ...
    if (
        ...
        raise ValueError("TRI100 four-spine probability manifest differs")
    ...
    if (
        ...
        raise ValueError("TRI100 four-spine probability shard lineage differs")
    data_path = Path(shard["data_path"])
    if not data_path.is_file() or sha256_file(data_path) != shard["data_sha256"]:
        raise ValueError("TRI100 four-spine probability bytes differ")
    arrays = load_npz_arrays(data_path)
    if set(arrays) != {"identity_digest", "probabilities"}:
        raise ValueError("TRI100 four-spine probability arrays differ")
    ...
    if (
        ...
        raise ValueError("TRI100 four-spine probability content differs")
...
def publish_probability_lock(
    ...
    for role, manifest in (
        ...
        if manifest.get("distribution_id") != distribution_id or manifest.get("role") != role:
            raise ValueError("TRI100 four-spine probability lock role differs")
    lock = artifact({
        "parents": hashes(parents),
        "distribution_id": distribution_id,
        "manifests": {
...
def validate_probability_lock(
    ...
    if (
        ...
        raise ValueError("TRI100 four-spine probability lock differs")
    ...
    for role in ("train", "validation"):
        ...
        if lock["manifests"].get(role) != manifest["content_hash"]:
            raise ValueError("TRI100 four-spine lock/manifest differs")
        manifests[role] = manifest
    return lock, manifests


@dataclass(frozen=True)
class SpineProbabilityTargets:
    ...
    def join(self, identity_digests: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            raise KeyError("TRI100 four-spine target join is incomplete") from error
        return np.ascontiguousarray(self.probabilities[indexes])


__all__ = [
    "SpineProbabilityTargets", "expected_temperature", "load_probability_role",
