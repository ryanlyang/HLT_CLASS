...
PARENT_IMPORT_CONTRACT: Final = f"{PREFIX}_PARENT_IMPORT/v1"
INTEGRATION_ATTESTATION_CONTRACT: Final = f"{PREFIX}_INTEGRATION_ATTESTATION/v1"
NODE_SPEC_CONTRACT: Final = f"{PREFIX}_NODE_SPEC/v1"
GRAPH_CONTRACT: Final = f"{PREFIX}_GRAPH/v1"
RECIPE_CONTRACT: Final = f"{PREFIX}_RECIPE/v1"
GRAPH_RECIPE_LOCK_CONTRACT: Final = f"{PREFIX}_GRAPH_RECIPE_LOCK/v1"
TARGET_SPEC_CONTRACT: Final = f"{PREFIX}_TARGET_SPEC/v1"
TARGET_GENERATION_CONTRACT: Final = f"{PREFIX}_TARGET_GENERATION/v1"
TARGET_SHARD_CONTRACT: Final = f"{PREFIX}_TARGET_SHARD/v1"
TARGET_MANIFEST_CONTRACT: Final = f"{PREFIX}_TARGET_MANIFEST/v1"
TARGET_CLEANUP_AUTHORIZATION_CONTRACT: Final = f"{PREFIX}_TARGET_CLEANUP_AUTHORIZATION/v1"
TARGET_CLEANUP_COMPLETION_CONTRACT: Final = f"{PREFIX}_TARGET_CLEANUP_COMPLETION/v1"
CALIBRATION_CONTRACT: Final = f"{PREFIX}_CALIBRATION/v1"
RESUME_STATE_CONTRACT: Final = f"{PREFIX}_RESUME_STATE/v1"
...
DEPLOYABLE_EXTRACTION_CONTRACT: Final = f"{PREFIX}_DEPLOYABLE_EXTRACTION/v1"
AGGREGATE_CONTRACT: Final = f"{PREFIX}_AGGREGATE/v1"
CAMPAIGN_SPEC_CONTRACT: Final = f"{PREFIX}_CAMPAIGN_SPEC/v1"
COMMAND_PLAN_CONTRACT: Final = f"{PREFIX}_COMMAND_PLAN/v1"
RUNTIME_BINDING_CONTRACT: Final = f"{PREFIX}_RUNTIME_BINDING/v1"
TASK_ATTESTATION_CONTRACT: Final = f"{PREFIX}_TASK_ATTESTATION/v1"
SUBMISSION_LEDGER_CONTRACT: Final = f"{PREFIX}_SUBMISSION_LEDGER/v1"
MONITOR_REPORT_CONTRACT: Final = f"{PREFIX}_MONITOR_REPORT/v1"
SOURCE_RECOVERY_CONTRACT: Final = f"{PREFIX}_SOURCE_RECOVERY/v1"
RESOURCE_RECOVERY_CONTRACT: Final = f"{PREFIX}_RESOURCE_RECOVERY/v1"
...
def build_artifact(contract: str, *, parents: Mapping[str, object], **payload: Any) -> dict[str, Any]:
    """Publish one strict v1 envelope with normalized immutable parents."""
...
__all__ = [name for name in globals() if name.endswith("_CONTRACT") or name in {
    ...
    "ROLE_COUNTS", "SMOKE_ROLE_COUNTS", "SOURCE_RECOVERY_PHRASE", "SUBMISSION_PHRASE",
