"""Exact-ledger monitoring and restart-zero recovery for TRI100 spine4."""
...
from .hcwdl_tri100_spine4_campaign import JOB_PREFIX, validate_campaign
...
def build_monitor(
    *, spec: Mapping[str, Any], ledger: Mapping[str, Any],
    ...
    if (
        ...
        raise ValueError("TRI100 spine4 monitor ledger differs")
    rows = []
    ...
    for task_id, job_id in ledger["jobs"].items():
        ...
        if path.is_file():
            try:
                ...
                valid = True
            except Exception as error:  # recorded; recovery retries it
    ...
    return artifact({
        ...
        "subject_spec_sha256": spec["content_hash"],
        "rows": rows, "exact_job_ids_only": True,
...
def validate_monitor(
    ...
    if (
        ...
        raise ValueError("TRI100 spine4 monitor differs")
    return digest
...
def _recovery_plan(
    ...
    worker = str(
        ...
        / "sbatch/run_hcwdl_tri100_spine4_recovery_task.sh"
    )
    commands = []
    for task in subject["tasks"]:
        ...
        if not set(task["dependencies"]) <= retry | completed:
            raise ValueError("TRI100 spine4 recovery parent coverage differs")
    ...
    return artifact({
        ...
        "commands": commands, "recovery": True,
...
def create_recovery(
    *, subject_spec: str | Path, subject_ledger: str | Path,
    ...
    if re.fullmatch(r"[0-9a-f]{40}", source_commit) is None:
        raise ValueError("TRI100 spine4 recovery commit differs")
    ...
    if set(ledger["jobs"]) != set(task_ids):
        raise ValueError("TRI100 spine4 recovery ledger coverage differs")
    rows = {row["task_id"]: row for row in monitor["rows"]}
    if set(rows) != set(task_ids):
        raise ValueError("TRI100 spine4 recovery monitor coverage differs")
    active = [
        name for name, row in rows.items()
        if row["disposition"] == "active_or_unknown"
    ]
    if active:
        raise PermissionError(f"TRI100 spine4 recovery has active jobs: {active}")
    completed = [
        name for name in task_ids if rows[name]["disposition"] == "complete"
    ]
    retry = [name for name in task_ids if name not in completed]
    if not retry:
        raise ValueError("TRI100 spine4 recovery has no failed task")
    root = Path(recovery_root).resolve()
    if root.exists():
        raise FileExistsError("TRI100 spine4 recovery root already exists")
    recovery = artifact({
...
def validate_recovery(value: Mapping[str, Any]) -> str:
    ...
    if (
        ...
        raise ValueError("TRI100 spine4 recovery semantics differ")
    plan = load_json(Path(value["recovery_root"]) / "command_plan.json")
    if plan != _recovery_plan(value, subject):
        raise ValueError("TRI100 spine4 recovery plan differs")
    return digest


def clean_incomplete_task_outputs(spec: Mapping[str, Any], task_id: str) -> None:
    task = next(
        (row for row in spec["tasks"] if row["task_id"] == task_id), None,
    )
    if task is None:
        raise KeyError("unknown TRI100 spine4 recovery task")
    ...
    for target in targets:
        ...
        if resolved != root and root not in resolved.parents:
            raise PermissionError("TRI100 spine4 recovery target escapes root")
        if resolved.is_dir():
