...
from .hcwdl_mhpe_contracts import (
    EXECUTION_LOCK_CONTRACT, FINALIST_LOCK_CONTRACT, FINAL_EVALUATION_CONTRACT,
    validate_execution_lock,
)
from .hcwdl_mhpe_graph import ENSEMBLE_COMPONENTS, FINALISTS
...
def run_sealed_final_evaluation(
    *, campaign_spec_path: str | Path, finalist_lock_path: str | Path,
    ...
    finalist_hash = validate_content_hash(finalist, expected_contract=FINALIST_LOCK_CONTRACT, expected_schema_version=1)
    if [row["node_id"] for row in finalist["entries"]] != list(FINALISTS):
    ...
    for node_id in ("M0paired", *ENSEMBLE_COMPONENTS["D000E"], "M1"):
        ...
        if node_id in ENSEMBLE_COMPONENTS["D000E"]:
            component_logits[node_id] = target.logits
    probability = uniform_probability_ensemble(component_logits, temperature=1)
    ensemble_metrics = classification_metrics(np.log(np.maximum(probability, 1e-30)), labels)
    stage_report = load_json(root / "reports/D000E_stage.json")
    if (finalist_by_id["D000E"]["report_sha256"] != stage_report.get("content_hash")
            or finalist_by_id["D000E"]["checkpoint_sha256"] != stage_report.get("content_hash")):
        raise ValueError("HCWDL-MHPE finalist ensemble lineage differs")
    insertion = list(FINALISTS).index("D000E")
    rows.insert(insertion, {"node_id": "D000E", "component_order": list(ENSEMBLE_COMPONENTS["D000E"]), "metrics": ensemble_metrics})
    if [row["node_id"] for row in rows] != list(FINALISTS):
        raise RuntimeError("HCWDL-MHPE final evaluation ordering differs")
    payload = with_content_hash({
        "contract": FINAL_EVALUATION_CONTRACT, "schema_version": 1,
