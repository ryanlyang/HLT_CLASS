"""Exact-ledger monitoring and restart-zero recovery for MT20."""
...
from .hcwdl_tri100_spine4_mt20_campaign import JOB_PREFIX, validate_campaign
...
def build_monitor(
    *, spec: Mapping[str, Any], ledger: Mapping[str, Any],
    ...
    if ledger.get("dry_run") is not False or ledger.get("campaign_spec_sha256") != spec["content_hash"]:
        raise ValueError("MT20 monitor ledger differs")
    rows = []
...
def validate_monitor(
    ...
    if (
        ...
        raise ValueError("MT20 monitor differs")
    return digest
...
def _recovery_plan(
    ...
    worker = str(
        Path(recovery["project_dir"]) / "sbatch/run_hcwdl_tri100_spine4_mt20_recovery_task.sh"
    )
    commands = []
    for task in subject["tasks"]:
        ...
        if not set(task["dependencies"]) <= retry | completed:
            raise ValueError("MT20 recovery parent coverage differs")
        ...
        command.extend((
            ...
            f"HCWDL_MT20_RECOVERY={recovery['spec_path']}," +
            f"HCWDL_MT20_TASK={task['task_id']}", worker,
        ))
        commands.append({
...
def create_recovery(
    *, subject_spec: str | Path, subject_ledger: str | Path,
    ...
    if re.fullmatch(r"[0-9a-f]{40}", source_commit) is None:
        raise ValueError("MT20 recovery commit differs")
    ...
    if set(ledger["jobs"]) != set(task_ids):
        raise ValueError("MT20 recovery ledger coverage differs")
    rows = {row["task_id"]: row for row in monitor["rows"]}
    active = [name for name in task_ids if rows[name]["disposition"] == "active_or_unknown"]
    if active:
        raise PermissionError(f"MT20 recovery has active jobs: {active}")
    ...
    if not retry:
        raise ValueError("MT20 recovery has no failed task")
    root = Path(recovery_root).resolve()
    if root.exists():
        raise FileExistsError("MT20 recovery root already exists")
    recovery = artifact({
...
def validate_recovery(value: Mapping[str, Any]) -> str:
    ...
    if (
        ...
        raise ValueError("MT20 recovery semantics differ")
    return digest
...
def clean_incomplete_task_outputs(spec: Mapping[str, Any], task_id: str) -> None:
    ...
    if task is None:
        raise KeyError("unknown MT20 recovery task")
    ...
    if task["kind"] == "preflight":
        ...
        targets = [
            root / "training" / task["node_id"],
            root / "targets" / task["node_id"],
        ]
    ...
    for target in targets:
        ...
        if resolved != root and root not in resolved.parents:
            raise PermissionError("MT20 recovery target escapes root")
        if resolved.is_dir():
