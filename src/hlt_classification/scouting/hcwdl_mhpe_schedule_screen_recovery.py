"""Exact failed/downstream recovery for the D066 schedule screen."""
...
def failed_downstream_closure(failed: Sequence[str]) -> tuple[str, ...]:
    ...
    if not selected or not selected <= set(graph):
        raise ValueError("schedule-screen failed task set differs")
    changed = True
    while changed:
        changed = False
...
def create_recovery(
    *, campaign_spec: str | Path, submission_ledger: str | Path,
    ...
    if source_commit != spec["source_commit"] or authorization_phrase != RECOVERY_PHRASE:
        raise PermissionError("schedule-screen recovery source/authorization differs")
    ...
    if (ledger.get("dry_run") is not False
        ...
        raise ValueError("schedule-screen recovery evidence differs")
    ...
    for sequence, task_id in enumerate(closure):
        ...
        command += [
            "--export=ALL," + (
                f"PROJECT_DIR={project},HCWDL_SCHEDULE_SCREEN_RECOVERY_SPEC={root / 'recovery_spec.json'},"
                f"HCWDL_SCHEDULE_SCREEN_TASK={task_id}"
            ),
            str(project / "sbatch/run_hcwdl_mhpe_schedule_screen_recovery_task.sh"),
        ]
...
def validate_recovery(value: Mapping[str, Any]) -> str:
    ...
    if (validate_campaign(spec, verify_source_tree=False) != value.get("campaign_spec_sha256")
        ...
        raise ValueError("schedule-screen recovery semantics differ")
    ...
    if (validate_submission_ledger(ledger) != value.get("submission_ledger_sha256")
        ...
        raise ValueError("schedule-screen recovery evidence changed")
    return digest
