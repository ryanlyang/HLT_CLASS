"""Compact probability banks owned only by the TRI60 dense extension."""
...
from .hcwdl_mhpe_tri60_dense_contracts import (
    ...
    PROBABILITY_SHARD_CONTRACT, artifact, hashes, validate_artifact,
)
from .hcwdl_mhpe_tri60_dense_graph import (
    ENSEMBLE_COMPONENTS, distribution_consumers,
)
...
def _identities(value: np.ndarray) -> np.ndarray:
    ...
    if result.dtype != np.uint8 or result.ndim != 2 or result.shape[1] != 32:
        raise ValueError("dense probability identities must be uint8 [rows,32]")
    if len({bytes(row) for row in result}) != len(result):
        raise ValueError("dense probability identities repeat")
    return result


def expected_temperature(distribution_id: str, role: str) -> float:
    if distribution_id not in ENSEMBLE_COMPONENTS:
        raise KeyError(f"unknown dense distribution: {distribution_id}")
    if role == "validation":
        return 1.0
    if role != "train":
        raise PermissionError("dense probability role must be train/validation")
    return 1.0 if distribution_id == "DX_M1E" or distribution_id.endswith("D000E") else 2.0
...
def publish_probability_role(
    ...
    if re.fullmatch(r"[0-9a-f]{40}", producer_commit) is None:
        raise ValueError("dense probability producer commit differs")
    components = ENSEMBLE_COMPONENTS[distribution_id]
    if tuple(component_logits) != components:
        raise ValueError("dense probability component order differs")
    identities = _identities(identity_digests)
    temperature = expected_temperature(distribution_id, role)
    ...
    if probabilities.shape != (len(identities), 15):
        raise ValueError("dense probability rows differ")
    lineage = {}
    for component in components:
        ...
        if not isinstance(item, Mapping) or set(item) != {
            ...
            raise ValueError("dense component lineage differs")
    ...
    shard = artifact({
        "parents": hashes(parents), "distribution_id": distribution_id,
        "role": role, "rows": len(identities),
        "data_path": str(data_path.resolve()), "data_sha256": sha256_file(data_path),
...
def load_probability_role(
    ...
    components = ENSEMBLE_COMPONENTS[expected_distribution_id]
    if (
        ...
        or manifest.get("temperature") != expected_temperature(expected_distribution_id, expected_role)
        ...
        raise ValueError("dense probability manifest differs")
    ...
    if (
        ...
        or not np.allclose(probabilities.sum(1, dtype=np.float64), 1, rtol=0, atol=2e-6)
        or {name: array_sha256(name, value) for name, value in arrays.items()} != shard["array_sha256"]
    ):
        raise ValueError("dense probability content differs")
...
def publish_probability_lock(
    ...
    for role, manifest in (("train", train_manifest), ("validation", validation_manifest)):
        ...
        if manifest.get("distribution_id") != distribution_id or manifest.get("role") != role:
            raise ValueError("dense probability lock role differs")
    lock = artifact({
        "parents": hashes(parents), "distribution_id": distribution_id,
        "manifests": {
...
def validate_probability_lock(
    ...
    if (
        ...
        raise ValueError("dense probability lock differs")
    ...
    for role in ("train", "validation"):
        ...
        if lock["manifests"].get(role) != manifest["content_hash"]:
            raise ValueError("dense probability lock/manifest differs")
        manifests[role] = manifest
    return lock, manifests


@dataclass(frozen=True)
class DenseProbabilityTargets:
    ...
    def join(self, identity_digests: np.ndarray) -> np.ndarray:
        identities = _identities(identity_digests)
        try:
            indexes = np.asarray([self.lookup[bytes(row)] for row in identities], dtype=np.int64)
        except KeyError as error:
            raise KeyError("dense probability target join is incomplete") from error
        return np.ascontiguousarray(self.probabilities[indexes])


__all__ = [
    "DenseProbabilityTargets", "expected_temperature", "load_probability_role",
