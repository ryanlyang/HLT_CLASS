"""Fit and target-bank execution for the TRI100 four-spine study."""
...
from .hcwdl_tri100_spine4_probability import (
    SpineProbabilityTargets, load_probability_role, publish_probability_lock,
...
def _student_caches(spec: Mapping[str, Any], *, node) -> tuple[Any, ...]:
    ...
    return (
        foundation, split_hash, selection_hash, caches, input_key,
    )
...
def _probability_targets(
    ...
    if distribution_id == SOURCE_DISTRIBUTION:
        ...
        if consumer_id not in source["authorized_probability_consumers"]:
            raise PermissionError("TRI100 four-spine U000 consumer is unauthorized")
        ...
        if lock["content_hash"] != row["lock_sha256"]:
            raise ValueError("TRI100 four-spine imported U000 bank changed")
    ...
    if consumer_id not in lock["consumers"]:
        raise PermissionError("TRI100 four-spine target consumer is unauthorized")
    return SpineProbabilityTargets.load(
...
def run_reducer(
    *, spec: Mapping[str, Any], distribution_id: str, device: str = "cuda",
    ...
    if components is None or len(components) != 1:
        raise KeyError("unknown TRI100 four-spine reducer")
    component = components[0]
    node = NODE_REGISTRY[component]
    _, _, _, caches, input_key = _student_caches(spec, node=node)
    sampler_seed = derive_seed(
        int(spec["replicate_seed"]), node.seed_alias + "/sampler",
    )
    started = time.monotonic()
