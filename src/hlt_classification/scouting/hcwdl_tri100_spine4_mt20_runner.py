"""Fit and per-model target-bank execution for the MT20 four spines."""
...
from .hcwdl_tri100_spine4_mt20_contracts import (
    FINAL_CHECKPOINT_CONTRACT, MIXTURE_REGISTRY_CONTRACT, RECIPE_CONTRACT,
    SELECTED_CHECKPOINT_CONTRACT, STAGE_REPORT_CONTRACT,
    TRAINING_REPORT_CONTRACT, artifact, validate_artifact,
)
from .hcwdl_tri100_spine4_mt20_execution import validate_execution_acceptance
from .hcwdl_tri100_spine4_mt20_graph import (
    ANCHOR_NODE_ID, EARLY_STOPPING, GRAPH_SHA256, LR_SCHEDULE, NODE_REGISTRY,
    PROBABILITY_COMPONENTS, TEACHER_DISTRIBUTIONS, TEACHER_NODES,
)
from .hcwdl_tri100_spine4_mt20_probability import (
    load_probability_role, materialize_ram_mixture, publish_probability_lock,
...
def _runtime(spec: Mapping[str, Any], *, node_id: str) -> Tri60TrainingRuntime:
    ...
    training = recipe["anchor_training" if node_id == ANCHOR_NODE_ID else "training"]
...
def _probability_targets(spec: Mapping[str, Any], *, node_id: str):
    banks = {}
    locks = {}
    parent_hashes = {}
    for distribution in TEACHER_DISTRIBUTIONS[node_id]:
        directory = distribution_output_dir(spec["campaign_root"], distribution)
        lock, _ = validate_probability_lock(
            directory / "lock.json", distribution_id=distribution,
        )
        if (
            node_id not in lock["consumers"]
            ...
            raise PermissionError("MT20 target-bank lineage/consumer differs")
        manifest, identities, probabilities = load_probability_role(
            directory / "train_manifest.json",
            expected_distribution_id=distribution, expected_role="train",
        )
        banks[distribution] = (manifest, identities, probabilities)
        locks[distribution] = lock
        parent_hashes[f"probability_lock/{distribution}"] = lock["content_hash"]
    targets, registry = materialize_ram_mixture(
        node_id=node_id, banks=banks, locks=locks,
        parents={"campaign_spec": spec["content_hash"], **parent_hashes},
    )
    registry_path = (
        Path(spec["campaign_root"]) / "targets" / node_id / "mixture_registry.json"
    )
    write_immutable_json(registry_path, registry)
    validate_mixture_registry(load_json(registry_path), node_id=node_id)
    return targets, registry, locks
...
def _model(spec: Mapping[str, Any], node_id: str, *, device: str):
    ...
    return load_tri60_model(path, device=device, authority=training_authority(node_id))
...
def run_fit(
    *, spec: Mapping[str, Any], node_id: str, device: str = "cuda",
    ...
    if node_id not in NODE_REGISTRY:
        raise KeyError("unknown MT20 fit")
    ...
    if node_id != ANCHOR_NODE_ID:
        targets, registry, locks = _probability_targets(spec, node_id=node_id)
    started = time.monotonic()
    _, split_hash, selection_hash, caches, input_key = _student_caches(spec, node=node)
    ...
    if node_id == ANCHOR_NODE_ID:
        ...
        parents["teacher_mixture_registry"] = registry["content_hash"]
        for teacher, distribution in zip(
            TEACHER_NODES[node_id], TEACHER_DISTRIBUTIONS[node_id], strict=True,
        ):
            report = load_json(
                node_output_dir(spec["campaign_root"], teacher) / "training_report.json"
            )
            parents[f"teacher_report/{teacher}"] = report["content_hash"]
            parents[f"teacher_checkpoint/{teacher}"] = report[
                "selected_checkpoint_sha256"
            ]
            parents[f"probability_lock/{distribution}"] = locks[distribution][
                "content_hash"
            ]
...
def run_reducer(
    *, spec: Mapping[str, Any], distribution_id: str, device: str = "cuda",
    ...
    if components is None or len(components) != 1:
        raise KeyError("unknown MT20 reducer")
    ...
    try:
        ...
        for role in ("train", "validation"):
            ...
            role_state[role] = {"identities": identities, "logits": logits, "labels": labels}
        del model
        gc.collect()
        try:
            import torch
        ...
        _, _, probabilities = load_probability_role(
            ...
            expected_distribution_id=distribution_id, expected_role="validation",
        )
        ...
        report_out = artifact({
            ...
            "distribution_id": distribution_id, "component_order": [component],
