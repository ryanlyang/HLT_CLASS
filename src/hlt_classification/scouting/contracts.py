    ...
    *,
    ...
    if role == "final_test" and branch_read:
        if shared_final_capability is not None:
            if (
                final_population_sha256 is None
                or final_task_id is None
            ...
            # This is the common gate beneath every ROOT-backed final reader.
            # Revalidate the exact frozen branch family here so a caller cannot
            # use a legitimate prediction/assignment capability to request a
            # label-bearing projection directly from ``streaming.py``.
        ...
        if shared_reservation_active:
            raise PermissionError(
                "legacy finalist/execution locks are invalid after shared reservation"
            )
