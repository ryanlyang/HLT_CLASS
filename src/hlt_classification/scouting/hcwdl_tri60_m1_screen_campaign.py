"""Source-pinned standalone TRI60 M1 compression-screen campaign."""
...
from .hcwdl_tri60_m1_screen_source import build_source_lock, validate_source_lock


CREATION_PHRASE: Final = "AUTHORIZE HCWDL TRI60 M1 COMPRESSION SCREEN EXACT SPEC"
SUBMISSION_PHRASE: Final = "SUBMIT HCWDL TRI60 M1 COMPRESSION SCREEN EXACT LEDGER"
JOB_PREFIX: Final = "hcwm1scr"
MINIMUM_FREE_DISK_BYTES: Final = 16 * 1024**3
SCHEDULER_NICE: Final = 5000
...
def campaign_tasks() -> list[dict[str, Any]]:
    ...
    if len(rows) != 23:
        raise RuntimeError("TRI60 M1 screen task count differs")
    return rows


def _command_plan(spec: Mapping[str, Any]) -> dict[str, Any]:
    commands = []
    worker = str(Path(spec["project_dir"]) / "sbatch/run_hcwdl_tri60_m1_screen_task.sh")
    for task in spec["tasks"]:
        ...
        command.extend((
            "--export=ALL," +
            f"PROJECT_DIR={spec['project_dir']},HCWDL_M1_SCREEN_SPEC={spec['spec_path']}," +
            f"HCWDL_M1_SCREEN_TASK={task['task_id']}",
            worker,
        ))
        commands.append({
...
def create_campaign(
    *, source_campaign_spec: str | Path, campaign_root: str | Path,
    ...
    if re.fullmatch(r"[0-9a-f]{40}", source_commit) is None:
        raise ValueError("TRI60 M1 screen source commit differs")
    if authorize_live_submission and authorization_phrase != CREATION_PHRASE:
        raise PermissionError("TRI60 M1 screen creation phrase differs")
    root = Path(campaign_root).resolve()
    project = Path(project_dir).resolve()
    if publish and root.exists():
        raise FileExistsError("TRI60 M1 screen root already exists")
    source_lock = build_source_lock(source_campaign_spec)
    graph = graph_payload()
    if validate_graph() != GRAPH_SHA256:
        raise RuntimeError("TRI60 M1 screen graph failed validation")
    paths = {
...
def validate_campaign(value: Mapping[str, Any], *, executable: bool = False) -> str:
    ...
    if (
        ...
        or value.get("condition_count") != 20
        or value.get("fresh_fit_count") != 19
