"""Exact failed/downstream recovery for endpoint-mixture campaigns."""
...
def failed_downstream_closure(failed: Sequence[str]) -> tuple[str, ...]:
    ...
    if not requested or not requested <= set(graph):
        raise ValueError("endpoint-mixture failed task set differs")
    changed = True
    while changed:
        changed = False
...
def create_recovery(*, campaign_spec: str | Path, submission_ledger: str | Path,
    ...
    if source_commit != spec["source_commit"] or authorization_phrase != RECOVERY_PHRASE:
        raise PermissionError("endpoint-mixture recovery source/authorization differs")
    ...
    if (ledger.get("dry_run") is not False or ledger.get("campaign_spec_sha256") != spec["content_hash"]
        ...
        raise ValueError("endpoint-mixture recovery ledger/monitor differs")
    ...
    by_id = {row["task_id"]: row for row in campaign_tasks()}
    for task_id in closure:
        ...
        command = ["sbatch", "--parsable", "--account=reu-aisocial", "--partition=tigris",
                   f"--cpus-per-task={resource['cpus']}", f"--mem={resource['memory']}",
                   f"--time={resource['walltime']}", f"--job-name=hcwmix_r_{task_id}"]
        if resource.get("gpu"): command += [f"--gres={resource['gpu']}", "--signal=B:USR1@120"]
        if dependencies: command.append("--dependency=afterok:" + ":".join(f"${{JOB_{name}}}" for name in dependencies))
...
def validate_recovery(value: Mapping[str, Any]) -> str:
    ...
    if (validate_campaign(spec, verify_source_tree=False) != value.get("campaign_spec_sha256")
        ...
        raise ValueError("endpoint-mixture recovery semantics differ")
    ledger = load_json(value["submission_ledger_path"]); monitor = load_json(value["monitor_report_path"])
    if (validate_submission_ledger(ledger) != value.get("submission_ledger_sha256")
            or validate_content_hash(monitor, expected_contract=MONITOR_CONTRACT, expected_schema_version=1) != value.get("monitor_report_sha256")):
        raise ValueError("endpoint-mixture recovery evidence changed")
    return digest
