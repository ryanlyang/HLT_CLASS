"""Exact-ledger monitoring and restart-zero recovery for the bottleneck control."""
...
from .hcwdl_tri100_spine4_bottleneck_campaign import JOB_PREFIX, validate_campaign
...
def build_monitor(
    *, spec: Mapping[str, Any], ledger: Mapping[str, Any],
    ...
    if ledger.get("dry_run") is not False or ledger.get("campaign_spec_sha256") != spec["content_hash"]:
        raise ValueError("bottleneck monitor ledger differs")
    rows = []
...
def validate_monitor(
    ...
    if (
        ...
        raise ValueError("bottleneck monitor differs")
    return digest
...
def _recovery_plan(
    ...
    retry = set(recovery["retry_tasks"]); completed = set(recovery["completed_tasks"])
    worker = str(
    ...
    for task in subject["tasks"]:
        ...
        if not set(task["dependencies"]) <= retry | completed:
            raise ValueError("bottleneck recovery parent coverage differs")
...
def create_recovery(
    *, subject_spec: str | Path, subject_ledger: str | Path,
    ...
    if re.fullmatch(r"[0-9a-f]{40}", source_commit) is None:
        raise ValueError("bottleneck recovery commit differs")
    ...
    if set(ledger["jobs"]) != set(task_ids):
        raise ValueError("bottleneck recovery ledger coverage differs")
    rows = {row["task_id"]: row for row in monitor["rows"]}
    if set(rows) != set(task_ids):
        raise ValueError("bottleneck recovery monitor coverage differs")
    active = [name for name, row in rows.items() if row["disposition"] == "active_or_unknown"]
    if active:
        raise PermissionError(f"bottleneck recovery has active jobs: {active}")
    ...
    if not retry:
        raise ValueError("bottleneck recovery has no failed task")
    root = Path(recovery_root).resolve()
    if root.exists():
        raise FileExistsError("bottleneck recovery root already exists")
    recovery = artifact({
...
def validate_recovery(value: Mapping[str, Any]) -> str:
    ...
    completed = value.get("completed_tasks", ()); retry = value.get("retry_tasks", ())
    if (
        ...
        raise ValueError("bottleneck recovery semantics differ")
    if load_json(Path(value["recovery_root"]) / "command_plan.json") != _recovery_plan(value, subject):
        raise ValueError("bottleneck recovery plan differs")
    return digest
...
def clean_incomplete_task_outputs(spec: Mapping[str, Any], task_id: str) -> None:
    ...
    if task is None:
        raise KeyError("unknown bottleneck recovery task")
    root = Path(spec["campaign_root"]).resolve(); targets: list[Path] = []
    ...
    for target in targets:
        ...
        if resolved != root and root not in resolved.parents:
            raise PermissionError("bottleneck recovery target escapes root")
        if resolved.is_dir():
