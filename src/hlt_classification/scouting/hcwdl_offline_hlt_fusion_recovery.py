"""Exact-ID monitoring and restart-zero recovery for fusion withdrawal."""
...
from .hcwdl_offline_hlt_fusion_campaign import JOB_PREFIX, validate_campaign
...
def build_monitor(*, spec, ledger, states_by_job_id):
    validate_campaign(spec)
    ledger_hash = validate_submission_ledger(ledger)
    all_ids = [row["task_id"] for row in spec["tasks"]]
    scopes = (all_ids, all_ids[:3], all_ids[3:])
    try:
        task_ids = next(rows for rows in scopes if set(rows) == set(ledger.get("jobs", {})))
    except StopIteration as error:
        raise ValueError("fusion monitor ledger coverage differs") from error
    if (
        ledger.get("dry_run") is not False
        or ledger.get("campaign_spec_sha256") != spec["content_hash"]
    ):
        raise ValueError("fusion monitor ledger differs")
    rows = []
    root = Path(spec["campaign_root"])
    for task_id in task_ids:
        job_id = str(ledger["jobs"][task_id])
        state = str(states_by_job_id.get(job_id, "UNKNOWN")).split()[0].split("+")[0].upper()
    ...
    return artifact({
        ...
        "scope_tasks": list(task_ids), "exact_job_ids_only": True,
        "poor_metrics_do_not_affect_disposition": True,
        "final_test_accessed": False,
    }, contract=MONITOR_CONTRACT)


def validate_monitor(value, *, spec_sha256, ledger_sha256):
    ...
    if (
        ...
        or not value.get("rows") or value.get("exact_job_ids_only") is not True
        or value.get("poor_metrics_do_not_affect_disposition") is not True
        or value.get("final_test_accessed") is not False
    ):
        raise ValueError("fusion monitor differs")
    return digest


def _recovery_plan(recovery: Mapping[str, Any], subject: Mapping[str, Any]):
    ...
    worker = str(
        ...
        / "sbatch/run_hcwdl_offline_hlt_fusion_recovery_task.sh"
    )
    commands = []
    for task in subject["tasks"]:
        task_id = task["task_id"]
        if task_id not in retry:
            continue
        if not set(task["dependencies"]) <= retry | completed | satisfied:
            raise ValueError("fusion recovery parents differ")
        ...
        command.extend((
            ...
            f"HCWDL_FUSION_RECOVERY={recovery['spec_path']}," +
            f"HCWDL_FUSION_TASK={task_id}", worker,
        ))
        commands.append({
...
def create_recovery(
    *, subject_spec: str | Path, subject_ledger: str | Path,
    ...
    if re.fullmatch(r"[0-9a-f]{40}", source_commit) is None:
        raise ValueError("fusion recovery commit differs")
    ...
    all_ids = [row["task_id"] for row in subject["tasks"]]
    scope = list(monitor["scope_tasks"])
    if scope not in (all_ids, all_ids[:3], all_ids[3:]):
        raise ValueError("fusion recovery scope differs")
    rows = {row["task_id"]: row for row in monitor["rows"]}
    active = [name for name in scope if rows[name]["disposition"] == "active_or_unknown"]
    if active:
        raise PermissionError(f"fusion recovery has active jobs: {active}")
    completed = [name for name in scope if rows[name]["disposition"] == "complete"]
    retry = [name for name in scope if name not in completed]
    if not retry:
        raise ValueError("fusion recovery has no failed task")
    satisfied = []
    for task in subject["tasks"]:
        ...
        for parent in task["dependencies"]:
            if parent in scope or parent in satisfied:
                continue
            path = task_attestation_path(subject["campaign_root"], parent, None)
    ...
    if root.exists():
        raise FileExistsError("fusion recovery root exists")
    recovery = artifact({
    ...
    write_immutable_json(root / "command_plan.json", _recovery_plan(recovery, subject))
    return recovery
...
def validate_recovery(value):
    ...
    subject_hash = validate_campaign(subject)
    all_ids = [row["task_id"] for row in subject["tasks"]]
    scope = list(value.get("scope_tasks", ()))
    if (
        value.get("parents", {}).get("subject_spec") != subject_hash
        or scope not in (all_ids, all_ids[:3], all_ids[3:])
        or set(value.get("completed_tasks", ())) | set(value.get("retry_tasks", ())) != set(scope)
        or set(value.get("completed_tasks", ())) & set(value.get("retry_tasks", ()))
        ...
        raise ValueError("fusion recovery semantics differ")
    return digest


def clean_incomplete_task_outputs(spec: Mapping[str, Any], task_id: str):
    task = next((row for row in spec["tasks"] if row["task_id"] == task_id), None)
    if task is None:
        raise KeyError("unknown fusion recovery task")
    root = Path(spec["campaign_root"]).resolve()
    targets = []
    if task["kind"] == "capacity_audit":
        targets = [Path(spec["artifact_paths"]["capacity_audit"])]
    ...
    elif task["kind"] == "teacher_bank":
        targets = [root / "probabilities" / "ANCHORED_FUSION_OH_T2"]
    elif task["kind"] == "extract":
        targets = [root / "deployable" / task["node_id"]]
    ...
    targets.append(task_attestation_path(root, task_id, None))
    targets.append(root / "output_inventories" / task_id / "single.json")
    for target in targets:
        resolved = target.resolve()
        if resolved != root and root not in resolved.parents:
            raise PermissionError("fusion recovery target escapes root")
        if resolved.is_dir():
