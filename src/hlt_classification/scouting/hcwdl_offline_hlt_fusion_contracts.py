"""Versioned contracts for the offline/HLT fusion-withdrawal campaign."""
...
PREFIX: Final = "HCWDL_OFFLINE_HLT_FUSION_WITHDRAWAL"
...
DEPLOYABLE_CHECKPOINT_CONTRACT: Final = f"{PREFIX}_DEPLOYABLE_CHECKPOINT/v1"
TEACHER_BANK_SHARD_CONTRACT: Final = f"{PREFIX}_TEACHER_BANK_SHARD/v1"
TEACHER_BANK_MANIFEST_CONTRACT: Final = f"{PREFIX}_TEACHER_BANK_MANIFEST/v1"
TEACHER_BANK_LOCK_CONTRACT: Final = f"{PREFIX}_TEACHER_BANK_LOCK/v1"
ALPHA_ZERO_AUDIT_CONTRACT: Final = f"{PREFIX}_ALPHA_ZERO_AUDIT/v1"
...
def artifact(payload: Mapping[str, Any], *, contract: str) -> dict[str, Any]:
    if contract not in CONTRACTS:
        raise ValueError("fusion-withdrawal artifact contract differs")
