"""Frozen B tracking interventions; original generators remain unchanged.

Traversal donor: c_diagnostic_generation.py at 5c18760f06fb5cb727668ce5482c2b6c35a73223.
Exact production replay guards this copy on every evaluated jet-replica.
All event detail is transient. Only bounded aggregates are published.
"""
...
from .c_diagnostic_generation import residual_draw as original_draw, cell_for, COORDINATES, check_replay

VARIANTS = ("FULL", "TRACK_CENTRAL", "DZ_CENTRAL", "DZERR_CENTRAL")
MASKS = {"FULL": (), "TRACK_CENTRAL": (4, 5, 6, 7), "DZ_CENTRAL": (5,), "DZERR_CENTRAL": (7,)}


def residual_draw(backend, condition, *, variant, **kwargs):
    if variant not in VARIANTS:
        raise ValueError("Unknown B tracking intervention")
    residual, info, detail = original_draw(backend, condition, variant="FULL", **kwargs)
    cell, _ = cell_for(backend, condition)
    detail["selected_residual_cell"] = "missing_category" if cell is None else ",".join(map(str, cell["key"]))
    if residual is not None:
        detail["raw_residual"] = residual.copy()
        residual = residual.copy()
        for j in MASKS[variant]:
            residual[j::8] = 0.
...
class TrackingGenerator(Generator):
    def __init__(self, response, variant):
        if variant not in VARIANTS:
            raise ValueError("Unknown B tracking intervention")
        if response["candidate_id"] != "B_L":
            raise ValueError("Tracking audit requires frozen B_L")
    ...
    def __call__(self, offline, *, jet, replica=0):
        ...
        def emit(indices, module, key):
            ...
            event["central"] = values.copy()
            event["response_limits"] = fitted["response_limits"]
            event["raw_increment"] = np.zeros_like(values)
            event["applied_increment"] = np.zeros_like(values)
            event["residual_available"] = False
            event["selected_residual_cell"] = "missing_state_backend"
            ...
            if backend is None:
                ...
                if residual is not None:
                    ...
                    event["residual_available"] = True
                    event["raw_increment"] = scale*event.pop("raw_residual")
                    event["applied_increment"] = scale*residual
...
def check_tracking_replay(expected, expected_info, actual, info, *, variant):
    check_replay(expected, expected_info, actual, info, full=variant == "FULL")
    ...
    np.testing.assert_array_equal(a.p4, b.p4, err_msg="Tracking intervention changed kinematics")
    unchanged = [j for j in range(4) if j+4 not in MASKS[variant]]
    np.testing.assert_array_equal(a.tracking[:, unchanged], b.tracking[:, unchanged],
                                  err_msg="Tracking intervention changed an unaffected field")
