...
from .concat_k2_campaign import COUNTS, artifact, registration, validate
...
def validate_launch(spec, *, check_source=True):
    ...
    if (spec["registration"] != registration(spec["registration"]["execution_site"]["partition"],
        ...
        raise ValueError("K2 concatenation launch lineage differs")
...
def build_import(launch):
    ...
    source = artifact("SOURCE_IMPORT", data_root=screen["data_root"],
        ...
        files={str(p): sha256_file(p) for p in files}, models_imported=[], assignments_imported=False, salience_formula_only=True, final_test_accessed=False)
    return source, foundation


def validate_import(source, *, deep=False):
    digest = validate(source, "SOURCE_IMPORT")
    if (source["models_imported"] != [] or source.get("assignments_imported") is not False
            or source.get("salience_formula_only") is not True or source["final_test_accessed"] is not False
