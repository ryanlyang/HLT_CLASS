"""Success-gated, exact-ID tier3 launchers; no mutations to parent campaigns."""
...
def _submit(spec, commands, directory, execute, authorization):
    if execute and authorization != AUTHORIZE:
        raise PermissionError("Exact dzfix fusion-chain tier3 authorization required")
...
def submit(spec, *, stage, execute=False, authorization=None):
    ...
    if execute and stage == "full":
        raise PermissionError("Full-DAG execution is forbidden; fresh tier3 gates must finish first")
    if execute and authorization != AUTHORIZE:
        raise PermissionError("Exact dzfix fusion-chain tier3 authorization required")
...
def _walltime_seconds(value):
    """Parse scontrol's finite [days-]hours:minutes:seconds representation."""
    match = re.fullmatch(r"(?:(\d+)-)?(\d+):([0-5]\d):([0-5]\d)", value)
    if match is None:
        raise PermissionError("Worker has no finite registered scheduler time limit")
...
def authenticate_job(spec, name, *, launch=False):
    ...
    if (fields.get("Account") != site["account"] or fields.get("Partition") != site["partition"]
            ...
            or _walltime_seconds(fields.get("TimeLimit", "")) != resource["minutes"] * 60
        ...
        raise PermissionError("Worker scheduler/environment differs from registered tier3 allocation")
    return job
