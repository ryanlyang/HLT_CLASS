...
def command(spec, name, resource, *, launch=False, dependencies=()):
    ...
    return result + [str(project / "sbatch/run_jetclass2_concat_k2.sh"),
...
def plan(spec, stage):
    ...
    chosen = {r["task_id"] for r in spec["tasks"] if stage == "full" or
              ((r["task_id"] in gates(spec)) == (stage == "gate"))}
    rows = []
...
def _submit(spec, commands, directory, execute, authorization):
    ...
    try:
        ...
        raise RuntimeError("Slurm submission failed; preserve the intent/receipt journal and reconcile before retrying. "
                           + str(error.stderr or error.stdout or error)) from error
    finally:
        claim.unlink()
...
def launcher_plan(launch, phase):
    ...
    if phase == "after_matching":
        ...
        if ledger["dry_run"] or ledger["campaign_spec_sha256"] != spec["content_hash"] or set(ledger["jobs"]) != set(gates(spec)):
            raise ValueError("Exact new-campaign gate ledger required")
        from .concat_k2_runtime import completed, science_gate
        if all(completed(spec, name) for name in gates(spec)):
            science_gate(spec)
        else:
...
def schedule(launch, *, phase="after_matching", execute=False, authorization=None):
    ...
    if (directory / "submission_ledger.json").is_file():
        ...
        from .concat_k2_campaign import validate
...
def authenticate_job(spec, name, *, launch=False):
    ...
    root = Path(spec["launch_root"] if launch else spec["campaign_root"])
    stage = name if launch else ("gate" if name in gates(spec) else "science")
    directory = root / ("submissions_" + stage)
    path = directory / "submission_ledger.json"
    # A very fast scheduler can start the worker before sbatch returns its ID
    # to the submitter. Wait only while an active submission claim exists.
    deadline=time.monotonic()+30
    journal=directory/"submission_ledger_journal"
    while (not path.is_file() and not list(journal.glob("*_"+name+".json"))
           and (directory/"submission_in_progress.claim").is_file() and time.monotonic()<deadline):
        time.sleep(.25)
    if path.is_file():
...
def monitor(spec):
    """Read-only accounting for exact submitted IDs in this campaign only."""
    validate_campaign(spec)
    rows=[]
    for stage in ("gate","science"):
        ...
        path=directory/"submission_ledger.json"
        if not path.is_file():
            if (directory/"submission_intents").exists():
                rows.append(dict(stage=stage,task_id="SUBMISSION_INCOMPLETE",job_id=None,state="RECONCILE_JOURNAL"))
            continue
        ledger=load_json(path); validate_submission_ledger(ledger)
        expected=plan(spec,stage)
        if (ledger["dry_run"] or ledger["campaign_spec_sha256"]!=spec["content_hash"]
                or set(ledger["jobs"])!={r["task_id"] for r in expected["commands"]}):
            raise ValueError("Monitor requires this exact live campaign stage")
