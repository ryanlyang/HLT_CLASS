...
from hlt_classification.cms2jc2_response import b_tracking_debug as debug
from hlt_classification.cms2jc2_response import dev_campaign as dev, dev_submission as submission
from hlt_classification.cms2jc2_response import b_tracking_worker as worker
...
def test_debug_uses_original_scientific_workers_and_fixed_resources():
    ...
    assert all(t["action"] == "bt_evaluate" and not t["depends_on"] for t in debug.tasks()[:4])
...
def test_retirement_refuses_active_or_successful_targets(state, monkeypatch):
    jobs = {task: str(i+100) for i, task in enumerate(("bt_acceptance", *debug.TARGETS))}
    status = {j: ("PENDING", "0:0") for j in jobs.values()}
    status[jobs["bt_acceptance"]] = ("COMPLETED", "0:0")
    status[jobs["bt_eval_0"]] = (state, "0:0")
    monkeypatch.setattr(submission, "states", lambda _: status)
    with pytest.raises(PermissionError, match="Preserve original B"):
        debug.old_states(dict(migration=dict(subject_jobs=jobs)))


def test_debug_replacement_lifecycle_and_unchanged_outputs(b_root, tmp_path, monkeypatch):
    subject, ctx, put, _, donor = b_root
    calls, statuses, pending_race = [], {}, {"mode": None}
    job_counter = [4100]
    ...
    def scheduler(argv):
        ...
        if argv[:3] == ["scontrol", "show", "partition"]:
            ...
            assert old_ledger["jobs"]["bt_acceptance"] not in argv
            for job in targets:
                statuses[job] = ("CANCELLED", "0:0")
            if pending_race["mode"]:
                statuses[old_ledger["jobs"]["bt_eval_0"]] = (pending_race["mode"], "0:0")
        else:
    ...
    old_ledger = submission.submit(subject, execute=True, authorization_phrase=dev.PHRASES["btrack"],
    ...
    put(subject, "bt_acceptance", dict(result=worker.acceptance(ctx, subject)))
    statuses[old_ledger["jobs"]["bt_acceptance"]] = ("COMPLETED", "0:0")
    ...
    for key, value in (("tasks", subject["tasks"]), ("variants", ["FULL"]), ("name", "other")):
    ...
    prior = len(calls)
    with pytest.raises(FileNotFoundError):
        submission.submit(spec, execute=True, authorization_phrase=dev.PHRASES["btrack"],
    ...
    pending_id = old_ledger["jobs"]["bt_eval_0"]
    with pytest.raises(PermissionError, match="allocation differs"):
        submission.scheduler_identity(subject, old_study, "bt_eval_0", pending_id)
    assert submission.scheduler_identity(subject, old_study, "bt_eval_0", pending_id,
    ...
    with monkeypatch.context() as m:
        def wrong_acceptance(s, owner, key):
            ...
            if s == subject and owner == "bt_acceptance":
    ...
    statuses[old_ledger["jobs"]["bt_eval_0"]] = ("CANCELLED", "0:0")
    ...
    prior = len(calls)
    statuses[old_ledger["jobs"]["bt_eval_0"]] = ("PENDING", "0:0")
    ...
    statuses[old_ledger["jobs"]["bt_eval_0"]] = ("CANCELLED", "0:0")
    prior = len(calls)
    ledger = submission.submit(spec, execute=True, authorization_phrase=dev.PHRASES["btrack"],
                               reviewed_plan_hash=plan["content_hash"])
    ...
    assert submission.submit(spec, execute=True, authorization_phrase=dev.PHRASES["btrack"],
    ...
    put(spec, "bt_report", dict(result=worker.report(ctx, spec)))
    from hlt_classification.cms2jc2_response.b_tracking_results import read
    assert read(spec)["jets"] == 12
    assert before == {p: sha256_file(p) for root in roots for p in root.rglob("*") if p.is_file()}
    # Success arriving in the old root must never be silently thrown away.
    put(subject, "bt_eval_0", dict(result=dev.product(spec, "bt_eval_0", "result")))
    with pytest.raises(PermissionError, match="Preserve completed B"):
        debug.validate_stage(spec)
