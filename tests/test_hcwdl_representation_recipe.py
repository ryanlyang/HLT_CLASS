...
def test_every_frozen_representation_value_is_explicit():
    ...
    assert values["relations"]["latent_state_source"] == "raw_particle_block_2"
    assert values["relations"]["latent_projection_applied"] is False
    assert values["relations"]["latent_normalization"] == "fp32_l2_eps_1e-12"
    assert values["relations"]["latent_statistic"] == (
        "cosine_similarity_of_raw_fp32_l2_normalized_particle_block_2_states"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("scientific_values", "relations", "latent_projection_applied"), True),
        (
            ("scientific_values", "relations", "latent_state_source"),
            "projected_particle_block_2",
        ),
