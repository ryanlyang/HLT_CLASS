...
def test_tri60_surface_fixture_uses_finite_timelike_unit_mass_vectors():
    # TRI60 does not import the standalone representation-campaign runtime
    # adapter.  Its production acceptance constructs the same three ordinary,
    # charged, and neutral vector surfaces directly through the model API.
    ordinary = _ordinary_inputs(batch=2, particles=6)
    charged = _ordinary_inputs(batch=2, particles=5, channels=19)
    neutral = _ordinary_inputs(batch=2, particles=4, channels=7)
    for vectors in (ordinary[1], charged[1], neutral[1]):
