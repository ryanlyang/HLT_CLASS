#!/usr/bin/env python3
"""Run one exact task from an authenticated HCWDL final recovery spec."""
...
def main() -> int:
    ...
    parser.add_argument("--task", choices=("sealed_final_evaluation", "aggregate_report"), required=True)
    args = parser.parse_args()
    spec = load_json(args.recovery_spec)
    validate_final_recovery_spec(spec, executable=True)
    evidence = validate_final_recovery_inputs(spec)
