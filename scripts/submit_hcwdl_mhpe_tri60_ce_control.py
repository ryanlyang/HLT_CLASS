#!/usr/bin/env python3
"""Dry-run or submit the isolated one-job M0CE60 control ledger."""
...
from hlt_classification.scouting.hcwdl_mhpe_tri60_ce_control import (  # noqa: E402
    SUBMISSION_PHRASE, validate_control,
)
from hlt_classification.scouting.hcwdl_mhpe_tri60_ce_control_contracts import (  # noqa: E402
...
def _expected(spec, plan, *, job_id: str, dry_run: bool):
    ...
    return build_submission_ledger(
        ...
        jobs={"train_M0CE60": job_id},
        commands={"train_M0CE60": command}, dry_run=dry_run,
    )
...
def main() -> int:
    ...
    validate_control(spec, executable=args.execute)
    canonical = Path(spec["campaign_root"]) / "control_spec.json"
    if args.spec.resolve() != canonical.resolve():
        raise PermissionError("CE60 submitter requires the canonical spec")
    ...
    if not args.execute:
        ...
        if args.output.exists() and load_json(args.output) != expected:
            raise FileExistsError("existing CE60 dry ledger differs")
        write_immutable_json(args.output, expected)
        return 0

    if args.authorization_phrase != SUBMISSION_PHRASE:
        raise PermissionError("CE60 submission phrase differs")
    ...
    if dry != _expected(spec, plan, job_id="1", dry_run=True):
        raise ValueError("CE60 canonical dry-run evidence differs")
    ...
    if args.output.exists():
        ...
        if (
            set(jobs) != {"train_M0CE60"}
            or existing != _expected(
                spec, plan, job_id=str(jobs.get("train_M0CE60", "")),
                dry_run=False,
            )
        ):
            raise FileExistsError("existing CE60 live ledger differs")
        return 0
    command = list(map(str, plan["commands"][0]["command"]))
    journal_path = (
        args.output.parent / f"{args.output.stem}_journal"
        / "0000_train_M0CE60.json"
    )
    if journal_path.is_file():
        ...
        expected_event = build_submission_event(
            campaign_spec_sha256=spec["content_hash"],
            task_id="train_M0CE60", job_id=job_id,
            command=command, sequence=0,
        )
        if event != expected_event:
            raise ValueError("CE60 submission journal differs")
    else:
