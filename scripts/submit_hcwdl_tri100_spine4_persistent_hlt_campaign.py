#!/usr/bin/env python3
"""Dry-run or submit the persistent-HLT-support four-spine DAG."""
...
def main()->int:
 ...
 if a.execute:
  if a.authorization_phrase!=SUBMISSION_PHRASE:raise PermissionError("persistent-HLT submission phrase differs")
  if ROOT.resolve()!=Path(spec["project_dir"]).resolve():raise PermissionError("persistent-HLT submitter is outside bound worktree")
