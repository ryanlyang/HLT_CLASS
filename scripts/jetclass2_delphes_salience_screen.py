...
def main():
    ...
    c=sub.add_parser("create"); c.add_argument("--bottleneck-root",type=Path,required=True); c.add_argument("--candidate-root",type=Path,action="append",required=True); c.add_argument("--resource-template",type=Path,required=True); c.add_argument("--data-root",type=Path,required=True); c.add_argument("--output-root",type=Path,required=True); c.add_argument("--source-commit",required=True)
    ...
    a=p.parse_args()
    if a.mode=="create": result=create_screen(bottleneck_root=a.bottleneck_root,candidate_roots=a.candidate_root,resource_template=a.resource_template,data_root=a.data_root,output_root=a.output_root,project=ROOT,source_commit=a.source_commit)
