#!/usr/bin/env python3
"""Dry-run or submit an exact tagged concatenation recovery DAG."""
...
from hlt_classification.scouting.hcwdl_offline_hlt_concat_campaign import RECOVERY_SUBMISSION_PHRASE  # noqa:E402
from hlt_classification.scouting.hcwdl_offline_hlt_concat_contracts import PLAN_CONTRACT,validate_artifact  # noqa:E402
from hlt_classification.scouting.hcwdl_offline_hlt_concat_recovery import validate_recovery  # noqa:E402
def main()->int:
 ...
 if a.execute:
  if a.authorization_phrase!=RECOVERY_SUBMISSION_PHRASE:raise PermissionError("tagged concatenation recovery phrase differs")
  if ROOT.resolve()!=Path(recovery["project_dir"]).resolve():raise PermissionError("tagged concatenation recovery submitter is outside bound worktree")
