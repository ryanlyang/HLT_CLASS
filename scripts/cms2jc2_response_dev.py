...
def main():
    ...
    for name in ("parent-spec", "project-dir", "source-commit", "root"):
        p.add_argument("--"+name, required=True)
    p = commands.add_parser("create-b-tracking-debug")
    for name in ("parent-spec", "project-dir", "source-commit", "root"):
        p.add_argument("--"+name, required=True)
    ...
    p = commands.add_parser("create-bdz-tuning")
    for name in ("parent-spec", "project-dir", "source-commit", "root"):
        p.add_argument("--"+name, required=True)
    ...
    if a.command == "create":
    ...
    elif a.command == "create-c-topology":
        ...
        result = create(parent_spec=a.parent_spec, project_dir=a.project_dir,
                        source_commit=a.source_commit, root=a.root)
    elif a.command == "create-b-tracking-debug":
        from hlt_classification.cms2jc2_response.b_tracking_debug import create
        result = create(parent_spec=a.parent_spec, project_dir=a.project_dir,
                        source_commit=a.source_commit, root=a.root)
    elif a.command == "create-c-topology-debug":
    ...
    elif a.command == "create-bdz-tuning":
        from hlt_classification.cms2jc2_response.bdz_campaign import create
        result = create(parent_spec=a.parent_spec, project_dir=a.project_dir,
                        source_commit=a.source_commit, root=a.root)
        ...
        if a.command == "retire-c-topology":
            ...
            print(json.dumps(result, indent=2, sort_keys=True))
            return 0
        if a.command == "retire-b-tracking":
            from hlt_classification.cms2jc2_response.b_tracking_debug import retire
            result = retire(spec, execute=a.execute, authorization_phrase=a.authorization_phrase,
                            reviewed_plan_hash=a.reviewed_plan_hash)
            print(json.dumps(result, indent=2, sort_keys=True))
            return 0
