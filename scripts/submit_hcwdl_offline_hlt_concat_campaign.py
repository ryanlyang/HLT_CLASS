#!/usr/bin/env python3
"""Dry-run or submit the isolated tagged concatenation DAG."""
...
from hlt_classification.scouting.hcwdl_offline_hlt_concat_campaign import SUBMISSION_PHRASE,validate_campaign  # noqa:E402
from hlt_classification.scouting.hcwdl_offline_hlt_concat_contracts import PLAN_CONTRACT,validate_artifact  # noqa:E402
from hlt_classification.scouting.hcwdl_offline_hlt_concat_contracts import CAPACITY_AUDIT_CONTRACT,EXECUTION_ACCEPTANCE_CONTRACT  # noqa:E402
...
def main()->int:
 ...
 if a.execute:
  if a.authorization_phrase!=SUBMISSION_PHRASE:raise PermissionError("tagged concatenation submission phrase differs")
  if ROOT.resolve()!=Path(spec["project_dir"]).resolve():raise PermissionError("tagged concatenation submitter is outside bound worktree")
  validate_source_checkout(ROOT,expected_commit=spec["source_commit"])
  if a.stage=="science":
   capacity=load_json(spec["artifact_paths"]["capacity_audit"]);acceptance=load_json(spec["artifact_paths"]["execution_acceptance"]);capacity_hash=validate_artifact(capacity,contract=CAPACITY_AUDIT_CONTRACT);validate_artifact(acceptance,contract=EXECUTION_ACCEPTANCE_CONTRACT)
   if acceptance.get("parents",{}).get("campaign_spec")!=spec["content_hash"] or acceptance.get("parents",{}).get("capacity_audit")!=capacity_hash:raise PermissionError("tagged concatenation science gates differ")
