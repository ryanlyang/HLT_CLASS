...
from hlt_classification.scouting.exploratory_test import submit_exploratory_test  # noqa: E402
...
def _run(command):
    ...
    if completed.returncode:
        ...
        raise RuntimeError(f"exploratory-test submission failed: {detail}")
...
def main() -> int:
    ...
    if args.execute and args.output.resolve() != expected:
        raise ValueError("execution ledger must be OUTPUT_ROOT/submission_ledger.json")
    if not args.execute and args.output.resolve() == expected:
        raise ValueError("dry-run output must differ from the execution ledger")
    validate_source_snapshot(
        spec["source_snapshot"], repository=REPO_ROOT, require_clean=True,
    )
    ledger = submit_exploratory_test(
        spec, spec_path=str(args.exploratory_test_spec.resolve()),
        dry_run=not args.execute, runner=_run if args.execute else None,
    )
