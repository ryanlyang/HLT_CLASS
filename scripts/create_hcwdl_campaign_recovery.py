#!/usr/bin/env python3
"""Create a source-pinned recovery for one failed primary HCWDL closure."""
...
from hlt_classification.scouting.hcwdl_campaign_recovery import (  # noqa: E402
    CAMPAIGN_RECOVERY_AUTHORIZATION_PHRASE, create_campaign_recovery_spec,
)
...
def main() -> int:
    ...
    if REPO_ROOT.resolve() != args.project_dir.resolve():
        raise PermissionError("campaign recovery must bind its creating checkout")
    if args.output.resolve() != (
        args.recovery_root / "recovery_spec.json"
    ).resolve():
        raise PermissionError("campaign recovery spec path is not canonical")
    ...
    if not args.authorize_live_submission and args.authorization_phrase is not None:
        raise ValueError("campaign recovery phrase supplied without live authorization")
    spec = create_campaign_recovery_spec(
    ...
    print(f"Parent mode: {spec['parent_mode']}")
    print(f"Retry tasks: {', '.join(spec['retry_tasks'])}")
    print(f"Failed jobs: {', '.join(spec['failed_job_ids'])}")
    print(f"Creation phrase: {CAMPAIGN_RECOVERY_AUTHORIZATION_PHRASE}")
    return 0
