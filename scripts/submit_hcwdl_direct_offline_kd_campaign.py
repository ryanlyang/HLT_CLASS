#!/usr/bin/env python3
"""Dry-run or submit the exact direct offline-to-HLT KD DAG."""
...
from hlt_classification.scouting.hcwdl_direct_offline_kd_graph import SUBMISSION_PHRASE  # noqa: E402
...
def main() -> int:
    ...
    canonical = (Path(spec["campaign_root"]) / "campaign_spec.json").resolve()
    if args.campaign_spec.resolve() != canonical:
        raise PermissionError("direct KD submitter requires the canonical specification")
    ...
    if args.authorization_phrase != SUBMISSION_PHRASE:
        raise PermissionError("direct KD submission phrase differs")
    if REPO_ROOT.resolve() != Path(spec["project_dir"]).resolve():
        raise PermissionError("direct KD submitter is not running from its bound worktree")
    ...
    if args.output.exists():
        ...
        if (
            ...
            raise FileExistsError("existing direct KD ledger belongs to another campaign")
        return 0
        ...
        for sequence, row in enumerate(plan["commands"]):
            ...
            if sequence < len(events):
                ...
                if event.get("task_id") != row["task_id"] or event.get("command") != command:
                    raise ValueError("direct KD submission journal differs")
