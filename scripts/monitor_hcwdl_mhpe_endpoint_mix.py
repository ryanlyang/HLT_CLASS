#!/usr/bin/env python3
"""Authenticate and summarize one endpoint-mixture submission ledger."""
...
from hlt_classification.scouting.hcwdl_mhpe_endpoint_mix import validate_campaign  # noqa:E402
...
def main()->int:
 ...
 if ledger["campaign_spec_sha256"]!=spec["content_hash"]:raise ValueError("endpoint-mixture monitor ledger/spec differs")
 ...
 for row in report["rows"]:print(f"{row['task_id']:<24} {row['job_id']:<12} {row['state']:<20} {row['disposition']}")
 return 0
