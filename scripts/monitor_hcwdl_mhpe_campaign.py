#!/usr/bin/env python3
"""Authenticate and summarize one HCWDL-MHPE ledger."""
...
def main()->int:
 ...
 if ledger["campaign_spec_sha256"]!=spec["content_hash"]:raise ValueError("MHPE monitor ledger/spec differs")
 ...
 for row in report["rows"]:print(f"{row['task_id']:<36} {row['job_id']:<12} {row['state']:<20} {row['disposition']}")
 return 0
