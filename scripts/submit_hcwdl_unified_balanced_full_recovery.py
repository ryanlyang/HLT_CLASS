#!/usr/bin/env python3
"""Dry-run or submit an exact HCWDL-UB-FULL3 recovery closure."""
...
from hlt_classification.scouting.hcwdl_unified_balanced_full_recovery import validate_recovery_command_plan,validate_recovery_spec  # noqa:E402
PHRASE="SUBMIT HCWDL UB FULL3 RECOVERY EXACT CLOSURE"
def main()->int:
 ...
 if not a.execute:
  ...
  if a.output.exists():
   if load_json(a.output)!=expected:raise FileExistsError("existing recovery dry-run ledger differs")
  else:write_immutable_json(a.output,expected)
  return 0
 if a.authorization_phrase!=PHRASE:raise PermissionError("HCWDL-UB-FULL3 recovery phrase differs")
 ...
 if a.output.exists():
  ...
  if ledger.get("dry_run") is not False or ledger.get("campaign_spec_sha256")!=spec["content_hash"]:raise FileExistsError("existing recovery ledger differs")
  return 0
 events=[];jobs={};journal=Path(spec["recovery_root"])/"submission_journal";existing=sorted(journal.glob("*.json")) if journal.is_dir() else []
 if len(existing)>len(plan["commands"]):raise ValueError("HCWDL-UB-FULL3 recovery journal is longer than its command plan")
 for sequence,path in enumerate(existing):
  row=plan["commands"][sequence]
  if path.name!=f"{sequence:04d}_{row['task_id']}.json":raise ValueError("HCWDL-UB-FULL3 recovery journal order differs")
  ...
  if event!=expected:raise ValueError("HCWDL-UB-FULL3 recovery journal event differs")
