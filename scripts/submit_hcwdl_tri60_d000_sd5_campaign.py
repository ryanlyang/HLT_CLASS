#!/usr/bin/env python3
"""Dry-run or submit the exact TRI60 D000 SD5 DAG with a durable journal."""
...
from hlt_classification.scouting.hcwdl_tri60_d000_sd5_campaign import (  # noqa: E402
    SUBMISSION_PHRASE, validate_campaign,
)
from hlt_classification.scouting.hcwdl_tri60_d000_sd5_contracts import (  # noqa: E402
    ...
    for index, item in enumerate(command):
        for parent in row["dependencies"]:
            if parent not in jobs:
                raise ValueError("TRI60 D000 SD5 submission lacks a dependency")
            item = item.replace(f"${{JOB_{parent}}}", jobs[parent])
        command[index] = item
    if any("${JOB_" in item for item in command):
        raise ValueError("TRI60 D000 SD5 command retains a dependency placeholder")
    return command
...
def _load_journal(journal: Path, spec, plan):
    ...
    if len(paths) > len(plan["commands"]):
        raise ValueError("TRI60 D000 SD5 submission journal has excess events")
    events = []
    jobs = {}
    for sequence, path in enumerate(paths):
        ...
        if event != expected or path.name != f"{sequence:04d}_{row['task_id']}.json":
            raise ValueError("TRI60 D000 SD5 submission journal differs")
        events.append(event)
...
def main() -> int:
    ...
    if args.spec.resolve() != canonical.resolve():
        raise PermissionError("TRI60 D000 SD5 submitter requires the canonical spec")
    ...
    if plan.get("spec_sha256") != spec["content_hash"]:
        raise ValueError("TRI60 D000 SD5 command plan belongs to another spec")
    expected_dry = _dry_ledger(spec, plan)
    if not args.execute:
        if args.output.exists() and load_json(args.output) != expected_dry:
            raise FileExistsError("existing TRI60 D000 SD5 dry-run ledger differs")
        if not args.output.exists():
            write_immutable_json(args.output, expected_dry)
        return 0
    if args.authorization_phrase != SUBMISSION_PHRASE:
        raise PermissionError("TRI60 D000 SD5 submission phrase differs")
    ...
    if not dry_path.is_file() or load_json(dry_path) != expected_dry:
        raise ValueError("TRI60 D000 SD5 canonical dry-run evidence differs")
    validate_submission_ledger(load_json(dry_path))
    if ROOT.resolve() != Path(spec["project_dir"]).resolve():
        raise PermissionError("TRI60 D000 SD5 submitter is outside the bound worktree")
    ...
    if args.output.exists():
        ...
        if set(ledger.get("jobs", {})) != set(_commands(plan)) or ledger != expected:
            raise FileExistsError("existing TRI60 D000 SD5 live ledger differs")
        return 0
