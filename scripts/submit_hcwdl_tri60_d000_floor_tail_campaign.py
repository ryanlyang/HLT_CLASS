#!/usr/bin/env python3
"""Dry-run or submit the matched D000 floor-tail confirmation DAG."""
...
from hlt_classification.scouting.hcwdl_tri60_d000_floor_tail_campaign import SUBMISSION_PHRASE, validate_campaign  # noqa: E402
from hlt_classification.scouting.hcwdl_tri60_d000_floor_tail_contracts import COMMAND_PLAN_CONTRACT, validate_artifact  # noqa: E402
    ...
    for index, item in enumerate(command):
        for parent in row["dependencies"]:
            if parent not in jobs:
                raise ValueError("D000 floor-tail submission lacks dependency")
            item = item.replace(f"${{JOB_{parent}}}", jobs[parent])
        command[index] = item
    if any("${JOB_" in item for item in command):
        raise ValueError("D000 floor-tail command retains placeholder")
    return command
...
def _load_journal(journal: Path, spec, plan):
    ...
    if len(paths) > len(plan["commands"]):
        raise ValueError("D000 floor-tail journal has excess events")
    events, jobs = [], {}
    for sequence, path in enumerate(paths):
        ...
        if event != expected or path.name != f"{sequence:04d}_{row['task_id']}.json":
            raise ValueError("D000 floor-tail journal differs")
        events.append(event)
...
def main() -> int:
    ...
    if args.spec.resolve() != canonical.resolve():
        raise PermissionError("D000 floor-tail submission requires canonical spec")
    ...
    if plan.get("spec_sha256") != spec["content_hash"]:
        raise ValueError("D000 floor-tail command plan belongs elsewhere")
    expected_dry = _dry_ledger(spec, plan)
    if not args.execute:
        if args.output.exists() and load_json(args.output) != expected_dry:
            raise FileExistsError("existing D000 floor-tail dry ledger differs")
        if not args.output.exists():
            write_immutable_json(args.output, expected_dry)
        return 0
    if args.authorization_phrase != SUBMISSION_PHRASE:
        raise PermissionError("D000 floor-tail submission phrase differs")
    ...
    if not dry_path.is_file() or load_json(dry_path) != expected_dry:
        raise ValueError("D000 floor-tail dry-run evidence differs")
    validate_submission_ledger(load_json(dry_path))
    if ROOT.resolve() != Path(spec["project_dir"]).resolve():
        raise PermissionError("D000 floor-tail submitter is outside worktree")
    ...
    if args.output.exists():
        ...
        if (
            ...
            raise FileExistsError("existing D000 floor-tail ledger differs")
        return 0
