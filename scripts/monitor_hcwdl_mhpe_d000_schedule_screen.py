#!/usr/bin/env python3
"""Authenticate and summarize a D000 teacher-distance screen ledger."""
...
from hlt_classification.scouting.hcwdl_mhpe_d000_schedule_screen import validate_campaign  # noqa:E402
...
def main():
 ...
 if ledger["campaign_spec_sha256"]!=spec["content_hash"]:raise ValueError("D000-screen monitor ledger/spec differs")
 ...
 for task in ledger["jobs"]:
  try:validate_task_attestation(load_json(task_attestation_path(spec["campaign_root"],task,None)),campaign_spec_sha256=spec["content_hash"],task_id=task,array_index=None);validity[task]=True
 ...
 for row in report["rows"]:print(f"{row['task_id']:<52} {row['job_id']:<12} {row['state']:<20} {row['disposition']}")
 return 0
