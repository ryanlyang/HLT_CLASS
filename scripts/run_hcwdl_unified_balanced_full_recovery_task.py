#!/usr/bin/env python3
"""Run one task from an authenticated HCWDL-UB-FULL3 recovery closure."""
...
from hlt_classification.scouting.hcwdl_unified_balanced_full_contracts import FOUNDATION_SPEC_CONTRACT  # noqa:E402
from hlt_classification.scouting.hcwdl_unified_balanced_full_recovery import validate_recovery_spec  # noqa:E402
...
def main()->int:
 ...
 if a.task not in recovery["task_ids"]:raise PermissionError("task is outside HCWDL-UB-FULL3 recovery closure")
 ...
 workflow=UnifiedBalancedFullFoundationWorkflow(scope,producer_commit=recovery["source_commit"],recovery_spec_sha256=recovery["content_hash"]) if scope["contract"]==FOUNDATION_SPEC_CONTRACT else UnifiedBalancedFullArmWorkflow(scope,producer_commit=recovery["source_commit"]);outputs=workflow.run(a.task,array_index=index)
