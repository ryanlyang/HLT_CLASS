#!/usr/bin/env python3
"""Dry-run or submit the exact 24-fit D000 teacher-distance screen DAG."""
...
from hlt_classification.scouting.hcwdl_mhpe_d000_schedule_screen import COMMAND_PLAN_CONTRACT,SUBMISSION_PHRASE,validate_campaign  # noqa:E402
...
def main():
 ...
 if a.spec.resolve()!=canonical.resolve():raise PermissionError("D000-screen submitter requires canonical spec")
 ...
 if not a.execute:
  ...
  if a.output.exists() and load_json(a.output)!=ledger:raise FileExistsError("D000-screen dry ledger differs")
  if not a.output.exists():write_immutable_json(a.output,ledger)
  return 0
 if a.authorization_phrase!=SUBMISSION_PHRASE:raise PermissionError("D000-screen submission phrase differs")
 dry=load_json(Path(spec["campaign_root"])/"dry_run_submission_ledger.json");validate_submission_ledger(dry);expected=build_submission_ledger(campaign_spec_sha256=spec["content_hash"],jobs={task:"1" for task in commands},commands=commands,dry_run=True)
 if dry!=expected:raise ValueError("D000-screen canonical dry run differs")
 validate_source_checkout(ROOT,expected_commit=spec["source_commit"])
 if a.output.exists():
  live=load_json(a.output);validate_submission_ledger(live)
  if live.get("dry_run") is not False or live.get("campaign_spec_sha256")!=spec["content_hash"]:raise FileExistsError("D000-screen live ledger differs")
  return 0
 ...
 for sequence,path in enumerate(existing):
  row=plan["commands"][sequence];event=load_json(path);command=_resolved(row,jobs);expected_event=build_submission_event(campaign_spec_sha256=spec["content_hash"],task_id=row["task_id"],job_id=event.get("job_id",""),command=command,sequence=sequence)
  if event!=expected_event:raise ValueError("D000-screen submission journal differs")
