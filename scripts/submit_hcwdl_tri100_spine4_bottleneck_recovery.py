#!/usr/bin/env python3
"""Dry-run or submit an exact bottleneck four-spine recovery DAG."""
...
from hlt_classification.scouting.hcwdl_tri100_spine4_bottleneck_campaign import RECOVERY_SUBMISSION_PHRASE  # noqa:E402
...
from hlt_classification.scouting.hcwdl_tri100_spine4_bottleneck_recovery import validate_recovery  # noqa:E402
def main()->int:
 ...
 if a.execute:
  if a.authorization_phrase!=RECOVERY_SUBMISSION_PHRASE:raise PermissionError("bottleneck recovery phrase differs")
  if ROOT.resolve()!=Path(recovery["project_dir"]).resolve():raise PermissionError("bottleneck recovery submitter is outside bound worktree")
