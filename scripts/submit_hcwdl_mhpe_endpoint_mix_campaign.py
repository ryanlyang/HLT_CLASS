#!/usr/bin/env python3
"""Dry-run or submit an exact endpoint-mixture campaign DAG."""
...
from hlt_classification.scouting.hcwdl_mhpe_endpoint_mix import COMMAND_PLAN_CONTRACT,SUBMISSION_PHRASE,validate_campaign  # noqa:E402
...
def main()->int:
 ...
 if not a.execute:
  ...
  return 0
 if a.authorization_phrase!=SUBMISSION_PHRASE:raise PermissionError("endpoint-mixture submission phrase differs")
 ...
 if dry!=expected:raise ValueError("endpoint-mixture canonical dry run differs")
 validate_source_checkout(ROOT,expected_commit=spec["source_commit"])
 if a.output.exists():
  live=load_json(a.output);validate_submission_ledger(live)
  if live.get("dry_run") is not False or live.get("campaign_spec_sha256")!=spec["content_hash"]:raise FileExistsError("endpoint-mixture live ledger differs")
  return 0
 ...
 for sequence,path in enumerate(existing):
  ...
  if event!=expected_event:raise ValueError("endpoint-mixture submission journal differs")
