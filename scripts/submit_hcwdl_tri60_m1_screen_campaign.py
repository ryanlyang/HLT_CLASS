#!/usr/bin/env python3
"""Dry-run or submit the exact TRI60 M1 compression-screen DAG."""
...
from hlt_classification.scouting.hcwdl_tri60_m1_screen_campaign import SUBMISSION_PHRASE, validate_campaign  # noqa: E402
    ...
    for index, item in enumerate(command):
        for parent in row["dependencies"]:
            if parent not in jobs:
                raise ValueError("TRI60 M1 screen submission lacks a dependency")
            item = item.replace(f"${{JOB_{parent}}}", jobs[parent])
        command[index] = item
    if any("${JOB_" in item for item in command):
        raise ValueError("TRI60 M1 screen command retains a placeholder")
    return command
...
def _load_journal(journal: Path, spec, plan):
    ...
    if len(paths) > len(plan["commands"]):
        raise ValueError("TRI60 M1 screen journal has excess events")
    events, jobs = [], {}
    for sequence, path in enumerate(paths):
        ...
        if event != expected or path.name != f"{sequence:04d}_{row['task_id']}.json":
            raise ValueError("TRI60 M1 screen journal differs")
        events.append(event)
...
def main() -> int:
    ...
    if args.spec.resolve() != (Path(spec["campaign_root"]) / "campaign_spec.json").resolve():
        raise PermissionError("TRI60 M1 screen requires the canonical spec")
    ...
    if not dry_path.is_file() or load_json(dry_path) != expected_dry:
        raise ValueError("TRI60 M1 screen canonical dry-run evidence differs")
    validate_submission_ledger(load_json(dry_path))
    if ROOT.resolve() != Path(spec["project_dir"]).resolve():
        raise PermissionError("TRI60 M1 screen submitter is outside its worktree")
    ...
    if args.output.exists():
        ...
        if (
            ...
            raise FileExistsError("existing TRI60 M1 screen live ledger differs")
        return 0
