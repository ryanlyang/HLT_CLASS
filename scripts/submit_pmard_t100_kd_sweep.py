#!/usr/bin/env python3
"""Dry-run or submit the supplemental T100 KD weight/temperature sweep."""
...
from hlt_classification.scouting.kd_sweep import submit_t100_sweep  # noqa: E402
...
def _run(command):
    ...
    if completed.returncode:
        ...
        raise RuntimeError(f"T100 sweep submission failed: {detail}")
...
def main() -> int:
    ...
    if not args.execute and args.output.resolve() == expected:
        raise ValueError(
            "dry-run output must differ from the immutable execution ledger path"
        )
    validate_source_snapshot(spec["source_snapshot"], repository=REPO_ROOT, require_clean=True)
    ledger = submit_t100_sweep(
        spec, spec_path=str(args.sweep_spec.resolve()), dry_run=not args.execute,
