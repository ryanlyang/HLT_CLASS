#!/usr/bin/env python3
"""Authenticate and summarize one exact HCWDL-UB submission ledger."""
...
def main()->int:
    ...
    if ledger["campaign_spec_sha256"]!=spec["content_hash"]: raise ValueError("HCWDL-UB monitor ledger/spec differs")
    ids=list(ledger["jobs"].values()); raw=subprocess.run(["sacct","-n","-P","-j",",".join(ids),"--format=JobIDRaw,State"],check=True,capture_output=True,text=True).stdout
    states={};
    ...
    for task,job in ledger["jobs"].items():
        ...
        for index in indexes:
            scope_root=spec.get("campaign_root",spec.get("recovery_root"))
            if scope_root is None: raise ValueError("HCWDL-UB monitor scope root differs")
            path=task_attestation_path(scope_root,task,index)
    ...
    for row in report["rows"]: print(f"{row['task_id']:<32} {row['job_id']:<12} {row['state']:<20} {row['disposition']}")
    return 0
