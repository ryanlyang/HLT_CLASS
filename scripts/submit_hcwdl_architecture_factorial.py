#!/usr/bin/env python3
"""Render or submit the exact HCWDL architecture-input factorial DAG."""
...
from hlt_classification.scouting.hcwdl_architecture_ablation import SUBMISSION_PHRASE  # noqa: E402
from hlt_classification.scouting.hcwdl_architecture_campaign import build_command_plan, validate_campaign  # noqa: E402
...
def main() -> int:
    ...
    if args.campaign_spec.resolve() != (Path(spec["campaign_root"]) / "campaign_spec.json").resolve():
        raise PermissionError("factorial submitter requires the canonical specification")
    ...
    if args.authorization_phrase != SUBMISSION_PHRASE:
        raise PermissionError("factorial submission phrase differs")
    if REPO_ROOT.resolve() != Path(spec["project_dir"]).resolve():
        raise PermissionError("factorial submitter is not running from its bound worktree")
    ...
    if args.output.exists():
        ...
        if (
            ...
            raise FileExistsError("existing factorial ledger is not this complete DAG")
        return 0
    ...
    if events:
        prior = assemble_submission_ledger(events, campaign_spec_sha256=spec["content_hash"])
    ...
    try:
        for sequence, row in enumerate(plan["commands"]):
            ...
            if sequence < len(events):
                ...
                if event.get("task_id") != row["task_id"] or event.get("command") != command:
                    raise ValueError("factorial submission journal differs")
                jobs[row["task_id"]] = str(event["job_id"]); continue
            ...
            event = build_submission_event(
                ...
                job_id=raw.split(";")[0], command=command, sequence=sequence,
            )
            write_immutable_json(journal / f"{sequence:04d}_{row['task_id']}.json", event)
            events.append(event); jobs[row["task_id"]] = str(event["job_id"])
    except BaseException:
        if events:
            partial = args.output.parent / f"{args.output.stem}_partial_{len(events):04d}.json"
            if not partial.exists():
                write_immutable_json(partial, assemble_submission_ledger(
