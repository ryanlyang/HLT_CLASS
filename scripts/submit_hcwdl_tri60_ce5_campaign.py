#!/usr/bin/env python3
"""Dry-run or submit the exact TRI60 CE5 DAG with a durable journal."""
...
from hlt_classification.scouting.hcwdl_tri60_ce5_campaign import (  # noqa: E402
    ...
    for index, item in enumerate(command):
        for parent in row["dependencies"]:
            if parent not in jobs:
                raise ValueError("TRI60 CE5 submission lacks a dependency")
            item = item.replace(f"${{JOB_{parent}}}", jobs[parent])
        command[index] = item
    if any("${JOB_" in item for item in command):
        raise ValueError("TRI60 CE5 command retains a dependency placeholder")
    return command
...
def _load_journal(journal: Path, spec, plan):
    ...
    if len(paths) > len(plan["commands"]):
        raise ValueError("TRI60 CE5 submission journal has excess events")
    events = []
    jobs = {}
    for sequence, path in enumerate(paths):
        ...
        if event != expected or path.name != f"{sequence:04d}_{row['task_id']}.json":
            raise ValueError("TRI60 CE5 submission journal differs")
        events.append(event)
...
def main() -> int:
    ...
    if args.spec.resolve() != canonical.resolve():
        raise PermissionError("TRI60 CE5 submitter requires the canonical spec")
    ...
    if not dry_path.is_file() or load_json(dry_path) != expected_dry:
        raise ValueError("TRI60 CE5 canonical dry-run evidence differs")
    validate_submission_ledger(load_json(dry_path))
    if ROOT.resolve() != Path(spec["project_dir"]).resolve():
        raise PermissionError("TRI60 CE5 submitter is outside the bound worktree")
    ...
    if args.output.exists():
        ...
        if set(ledger.get("jobs", {})) != set(_commands(plan)) or ledger != expected:
            raise FileExistsError("existing TRI60 CE5 live ledger differs")
        return 0
