# HCWDL Matching-Free Privileged Representation-KD Dense Descents

Status: **the four-track dense scientific/data plane is implemented locally.
It imports only the authenticated historical native-offline teacher; it does
not require or authorize the canceled base-HCWDL parent-prefix graph. Before
the dense smoke may be submitted, the final pushed representation commit must
produce four genuine Tigris resource measurements plus the dependent accounting
collector, a maximum-topology serialized storage template, a conservative
storage estimate, and a strict non-submitting executable-candidate audit.
Those probes and the later dense smoke each require their own exact phrase
authorization. A completed dense smoke is required before a 300k/100k pilot
candidate may be built. This document itself authorizes no scheduler mutation
or final-role access**.

## Controlling dense-descent amendment (2026-08-10)

This amendment supersedes every conflicting M1--M6 ascent, dual-teacher,
M5-control, terminal-M6, graph-count, and contract-version statement later in
this file.  The older material is retained only as design history.  It is not
an executable alternative and must not be used to build artifacts.

The active experiment is four dense downward ladders:
...
`TOFF` is the native-offline teacher.  The two strategy-specific D100 roots
are separately trained, because RSET and RREL have different representation
objectives.  At every subsequent rung the immediate same-strategy,
same-track predecessor supplies the sole detached logit target and the sole
representation target.  There is no second logit teacher.

Cold nodes use a fresh backbone at every rung.  Warm nodes load only the
immediate predecessor's authenticated selected model state and always reset
optimizer and scheduler state.  Both tracks run through D0 and stop after M1;
M2--M6 are absent.  D100 through D5 are deliberately nondeployable repaired-
view training intermediates.  Their authenticated model states may teach the
next rung.  D0 and M1 consume HLT inputs only and are deployable; the four M1
nodes are the terminal primary results.

The immutable graph contains exactly 86 nodes: two shared strategy roots plus
twenty D95--D0 and one M1 node for each of four strategy/track branches.  The
retired four M5 controls are not reused; the active control registry is empty.
Screening still retains every finite registered result and confirmation reruns
the four terminal M1 objectives at seeds 11, 22, 33, 44, and 55.

The active semantic identities are:

- `HCWDL_REPRESENTATION_DENSE_DESCENT_GRAPH/v1`;
- `HCWDL_REPRESENTATION_RECIPE/v5`, which explicitly binds RREL relation
  geometry to raw FP32 L2-normalized `particle_block_2` states and forbids a
  trainable projection in that component;
- the target-spec/logical/registry/generation/shard/manifest family `/v3`;
- training report, selected/final checkpoint, and model extraction `/v2`;
- screen aggregate `/v3`, with confirmation registry/run/aggregate `/v2`;
- campaign spec and command plan `/v3`;
- dense resource profile `/v1` or its accounting-only compatible wrapper
  `HCWDL_REPRESENTATION_DENSE_COMPATIBLE_RESOURCE_PROFILE/v1`, resource-probe
  plan/authorization/ledger, bounded collector-recovery authorization/ledger,
  serialized storage template, and dense storage estimate `/v1`;
- finalist/execution/prediction/evaluation/metric/final aggregate `/v2`.

Every Slurm job generated by this representation campaign uses the visibly
distinct `hcwdlr_` prefix.  The earlier completed 089b472 base-HCWDL smoke is
not invalidated or rerun by this representation-only change.  A genuine
production-worker representation miniature from the final pushed
representation commit is the exact 86-node dense smoke itself. The bounded
512/256/0 smoke registers 261 tasks and no final-role task; the 300k/100k/0
pilot registers 275 tasks, adds five-seed confirmation for only the four M1
endpoints, and still registers no final-role task. This amendment authorizes
implementation only; it authorizes no resource probe, dense smoke, pilot, or
final-role access.

The v1 ten-action non-final authority specified in the historical sections
below targeted the retired direct-D0/M1 graph.  It cannot authorize this v4
dense-descent recipe because it does not execute the D100--D5 repaired-view
chain.  Dense-descent Tigris acceptance must receive a new versioned authority;
no v1 authority or result may be relabeled.

The dense launch sequence is closed and non-circular. A non-executable smoke
planning spec first freezes the source, teacher import, recipe, graph and
512/256/0 population. Four scalar measurement jobs exercise exactly
`cpu_small`, `cpu_io`, `gpu_target` and `gpu_representation`; a fifth scalar
collector job depends on all four and captures their exact `sacct` bytes.
If that collector alone fails because a reviewed Slurm accounting field was
removed and its job ID is later purged, one separately phrase-authorized
replacement collector may reopen the original ledger and completed probe IDs.
It records zero measurement jobs, cannot rerun a probe, and cannot submit the
dense graph or access a final role.
When reviewed collector/parser repairs advance the Git commit after the four
probes, the compatible resource-profile wrapper reopens the exact base profile
and recovery authority/ledger, freezes the complete Git changed-path set, and
proves that no target, model, training, data, graph, or worker source changed.
This is the only permitted cross-commit reuse; the probes are not rerun merely
because their accounting collector required a site-interface repair.
The `gpu_representation` probe also serializes the largest `RREL_D100` model,
initialized AdamW moments, pilot-sized identity registry, selected checkpoint
and deployable checkpoint using the installed production model. The dense
storage estimate scales those measured templates over all 86 nodes, reserves
one committed plus one interrupted target generation, retains metadata for all
83 target generations, adds a fixed campaign reserve and 50% filesystem
headroom, and assigns zero bytes to final-role outputs. Candidate review and
live submission independently recheck that the target filesystem has at least
the resulting minimum free bytes. The old fixed-size inventory's
`final_assignment` category is not evidence for this no-final graph and is
