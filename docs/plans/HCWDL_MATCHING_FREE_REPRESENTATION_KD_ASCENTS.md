...
- the target-spec/logical/registry/generation/shard/manifest family `/v2`;
- training report, selected/final checkpoint, and model extraction `/v2`;
- screen, confirmation registry/run/aggregate `/v2`;
- campaign spec and command plan `/v2`;
...
representation commit remains required before representation campaign
submission.  This amendment authorizes implementation only; it authorizes no
scheduler mutation, parent-prefix submission, pilot, or final-role access.

The v1 ten-action non-final authority specified in the historical sections
below targeted the retired direct-D0/M1 graph.  It cannot authorize this v3
